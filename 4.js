...
function calendar_handler() {
    ...
    that.loadDateAndTimeFromInput = function() {
        if (that.$hiddenInput.val().length === 0)
            return;
        switch (that.type) {
        case 'datePicker':
...
function CalendarWidget(settings) {
    ...
    that.init = function() {
        ...
        that.$rootContainer = settings.$rootContainer;
    ...
    that.setDate = function(date) {
        var dayObj = dayjs(date);
        dateTXT = dayObj.format(that.calendarSettings.format);
        that.$fakeInput.html(dateTXT + ' (' + that.getDateDayName(date) + ')');
        that.$hiddenInput.val(dateTXT);
    }
    ;
    that.addPluginAbilityToInput = function() {
            ...
            confirm: {
                ...
                callback: function() {
                    ...
                    case 'timePicker':
                        that.onSubmit.call($modal, that.$time.val());
                        if (that.hoursInputSettings)
                            that.HoursInput.refreshBusinessHours();
                        break;
                    case 'dateTimePicker':
                        var selectedDate = that.$calendar.datepicker('getDate');
                        if (!selectedDate)
    ...
    that.getMessageBody = function() {
        var html = '';
        html += `<link rel="stylesheet" type="text/css" href="${$GLOBALS["cdn-system-files"]}/files/vendor/lw_sys/calendarHandler/css/calendarHandlerWidget.css?v=${$GLOBALS['v-cache']}" crossorigin="anonymous">`;
        html += '<div class="row calendar-handler-container">';
        switch (that.type) {
        case 'datePicker':
            html += '<div class="col-xs-12 date-picker-container"></div>';
    ...
    that.datePickerSubmit = function(calendar, modal) {
        calendar.off('changeDate').on('changeDate', function(ev) {
            ...
            if (!selectedDate)
                selectedDate = new Date();
            var dayObj = dayjs(selectedDate);
            formattedSelectedDate = dayObj.format(that.calendarSettings.format);
            that.onSubmit.call(modal, formattedSelectedDate, selectedDate);
            if (that.hoursInputSettings)
                that.HoursInput.refreshBusinessHours();
            modal.modal('hide');
        });
    ...
    ;
    that.getDateDayName = function(dateTXT) {
        let dayObj = dayjs(dateTXT);
        let calendarTranslationsOBJ = that.getCalendarTranslations();
        dayNameTXT = calendarTranslationsOBJ.days[dayObj.day()];
        return dayNameTXT;
    }
    ;
    ...
    that.HoursInput = function() {
        ...
        _.init = function(settings) {
            ...
            _.$rootContainer = settings.$rootContainer;
        ...
        _.refreshBusinessHours = function() {
            ...
            if (_.$tableHour.data('tomselect') && !_.isBackgroundCheck) {
            ...
            if (_.moduleTypeNUM == 96) {
                $.ajax({
                    ...
                    if (_.$tableHour.text().length <= 0) {
                        ...
                        if (typeof UiRichSelect !== 'undefined') {
                            new UiRichSelect();
                        }
                    }
                    if (!_.isInitialized) {
                    ...
                    if (_.onHoursLoad)
                        _.onHoursLoad.call(this, _.$tableHour);
                });
            } else {
                buildHourSelectBox();
                if (typeof UiRichSelect !== 'undefined') {
                ...
                if (_.serviceType == 'Classes') {
                    ...
                    $.each(_.$tableHour.find('option'), function(index, option) {
                        ...
                        $.each(unavailableHours, function(index, date) {
                            if (date.isExternalCalendar && date.isAllDayEvent && selectedDate == dayjs(date.orderDate).format("YYYY-MM-DD")) {
                    ...
                    for (var i = 0; i < hoursToRemove.length; i++) {
                        ...
                        $.each(_.$tableHour.find('option'), function(index, option) {
                            ...
                            if ((timeFromOption >= unavailableTime && timeFromOption < (unavailableTime + serviceDurationInMili)) || (timeFromOption > (unavailableTime - serviceDurationInMili) && timeFromOption < unavailableTime)) {
                                $(option).remove();
                            }
                        });
                    }
                    ...
                    $.each(unavailableHours, function(index, date) {
                        let orderDate = dayjs(date.orderDate).valueOf();
                        let serviceDuration = serviceDurationInMili;
                        if (date.isExternalCalendar) {
                            serviceDuration = dayjs(date.endOrderDate).valueOf() - orderDate;
                        }
                        $.each(_.$tableHour.find('option'), function(index, option) {
                            var timeFromOption = dayjs(selectedDate + ' ' + option.value).valueOf();
                            if ((timeFromOption >= orderDate && timeFromOption < (orderDate + serviceDuration)) || (timeFromOption > (orderDate - serviceDuration) && timeFromOption < orderDate)) {
                                $(option).remove();
                            }
                        });
                    });
            ...
            function buildHourSelectBox() {
                ...
                for (var i = 0; i <= that.maxShiftsAmount; i++) {
                    ...
                    for (; startTime <= (endTime - serviceDurationHourMili - serviceDurationMinutesMili - timeBetweenService); startTime += serviceInterval) {
                        ...
                        if (!hourExists)
                            _.$tableHour.append('<option value="' + newHour + ':' + newMinutes + '">' + changeTimeFormat(_.calendarSettings.timeFormat, newdate) + '</option>');
                        else
                            continue;
                    }
                }
                ...
                _.$tableHour.off('change').on('change', function(event) {
                    if (_.onHoursChage)
                        _.onHoursChage.call(this, $(this).val());
                });
            }
        }
        ;
...
function CalendarWidgetV2(settings) {
    var that = this;
    that.init = function() {
        that.maxShiftsAmount = settings.maxShiftsAmount ? settings.maxShiftsAmount : 3;
        that.$fakeInput = settings.$fakeInput;
        ...
        that.moduleID = settings.moduleID ? settings.moduleID : false;
        that.moduleTypeNUM = settings.moduleTypeNUM ? settings.moduleTypeNUM : false;
        that.itemID = settings.itemID ? settings.itemID : '';
        that.isAdmin = settings.isAdmin ? settings.isAdmin : false;
        that.$moduleSelector = settings.$moduleSelector ? settings.$moduleSelector : false;
        that.$rootContainer = settings.$rootContainer;
        that.calendarModalParent = settings.calendarModalParent ? settings.calendarModalParent : window;
        that.schedulesOffset = settings.schedulesOffset;
        if (that.$fakeInput.length === 0)
            return;
        that.addPluginAbilityToInput();
        if (!that.calendarSettings.isServiceUnavailable && that.calendarSettings) {
            if (that.calendarSettings.businessHours) {
                initializeBusinessHours();
            }
            that.calendarSettings.originalFormat = that.calendarSettings.format;
            ...
            if (that.hoursInputSettings) {
                that.originalOnHoursLoad = that.hoursInputSettings.onHoursLoad;
                that.hoursInputOriginalSettings = {
                    calendarSettings: that.calendarSettings,
                    hoursInputSettings: that.hoursInputSettings,
                    $moduleSelector: that.$moduleSelector,
                    moduleID: that.moduleID,
                    moduleTypeNUM: that.moduleTypeNUM,
                    $rootContainer: that.$rootContainer,
                    serviceDuration: that.$rootContainer.find('.serviceDuration').val(),
                    timeBetweenService: that.$rootContainer.find('.timeBetweenService').val(),
                    $serviceDatesContainer: that.$rootContainer.find('.serviceDatesContainer'),
                    $serviceHourContainer: that.$rootContainer.find('.serviceHourContainer'),
                    $tableHour: that.$rootContainer.find('.service-hour'),
                    $input: that.$rootContainer.find('.real-input.c-h-input'),
                    onInit: function() {
                        if (that.onInit)
                            that.onInit.call(this, that);
                    }
                };
                initializeQuickAvailableDays(that.hoursInputOriginalSettings, '');
                that.calendarUniqueID = that.$rootContainer.data('calendar-uniqueID');
                if (!that.calendarUniqueID) {
                    that.calendarUniqueID = 'calendarUniqueID' + Date.now();
                    that.$rootContainer.data('calendar-uniqueID', that.calendarUniqueID);
                }
                $(window).off('resize.' + that.calendarUniqueID).on('resize.' + that.calendarUniqueID, function() {
                    clearTimeout(that.resizeDelay);
                    that.resizeDelay = setTimeout(function() {
                        initializeQuickAvailableDays(that.hoursInputOriginalSettings, '');
                    }, 200);
                });
            } else {
    ...
    ;
    function initializeQuickAvailableDays(hoursInputOriginalSettings, selectedDateInCalendarByUser) {
        let maxButtonsToShow = 4;
        ...
        const $container = that.$rootContainer.find('.quick-available-days-container');
        that.$rootContainer.find('.serviceDatesContainer').removeClass('hidden');
        $container.hide();
        const originalDate = that.$hiddenInput.val();
        let startDate;
        if (originalDate && originalDate.trim() !== '') {
            const formattedDate = that.getDateAsIso(originalDate);
            startDate = dayjs(formattedDate);
        } else {
            startDate = dayjs();
        }
        const maxDateToCheck = startDate.add(that.schedulesOffset.maxDate, 'day');
        const firstDate = startDate.format('YYYY-MM-DD');
        const formattedFirstDate = dayjs(firstDate).format(that.calendarSettings.format);
        that.$hiddenInput.val(formattedFirstDate);
        let daysToShow = findAvailableDays(0, [], startDate);
        updateQuickDayButtons(daysToShow, hoursInputOriginalSettings);
        if (that.originalOnHoursLoad)
            that.originalOnHoursLoad.call(this, that, that.HoursInput.$tableHour);
        if (that.onInit)
            that.onInit.call(this, that);
        function findAvailableDays(index, daysToShow, baseDate) {
            const dateToCheck = baseDate.add(index, 'day');
            const formattedDate = dateToCheck.format('YYYY-MM-DD');
            const formatWithoutYear = that.calendarSettings.format.replace('/YYYY', '').replace('YYYY/', '');
            const displayDate = dateToCheck.format(formatWithoutYear);
            const dayName = that.getDateDayName(formattedDate);
            const dayOfWeek = dateToCheck.day();
            if (!['fullTime', 'bySchedule'].includes(that.calendarSettings.availability)) {
                if (!that.calendarSettings.businessHours[dayOfWeek].isActive) {
                    return findAvailableDays(index + 1, daysToShow, baseDate);
                }
            }
            let dayToCheck = dateToCheck.format(that.calendarSettings.format);
            that.$hiddenInput.val(dayToCheck);
            hoursInputOriginalSettings.hoursInputSettings.onHoursLoad = function(chWidget, $tableHour) {
                let isDayAvailable = $tableHour.find('option:not([value=""])').length > 0;
                if (isDayAvailable) {
                    daysToShow.push({
                        date: formattedDate,
                ...
                if (daysToShow.length >= maxButtonsToShow)
                    return;
                if (dateToCheck.isAfter(maxDateToCheck))
                    return;
                findAvailableDays(index + 1, daysToShow, baseDate);
            }
            ;
            that.HoursInput.init({
                ...hoursInputOriginalSettings,
                isQuickDaysValidator: true
            });
            return daysToShow;
        }
        function updateQuickDayButtons(daysToShow, hoursInputOriginalSettings) {
            if (daysToShow.length == 0) {
                that.$rootContainer.find('.ch-tools-container').addClass('no-quick-days')
                if (originalDate && originalDate.trim() !== '') {
                    that.$hiddenInput.val(originalDate);
                }
                that.HoursInput.init({
                    ...hoursInputOriginalSettings,
            ...
            daysToShow.forEach(day => {
                newButtonsHtml += `<button type="button" class="btn btn-default quick-day-btn" data-date="${day.date}">
...
<div class="quick-day-btn-name">${day.dayName}</div>
            ...
            if ($container.find('.quick-day-btn').length > 0) {
                $container.show();
                $container.parent().attr('data-quick-days-count', $container.find('.quick-day-btn').length);
            }
            $container.find('.quick-day-btn').on('click', function() {
                const selectedDate = $(this).data('date');
                const formattedDate = dayjs(selectedDate).format(that.calendarSettings.format);
                that.$hiddenInput.val(formattedDate);
                that.$fakeInput.html('<div class="date-value">' + formattedDate + '</div><div class="day-name">(' + that.getDateDayName(selectedDate) + ')</div>');
                $container.find('.quick-day-btn').removeClass('active');
                ...
                that.HoursInput.$input.val(formattedDate);
                that.HoursInput.$input.trigger('change');
            });
            document.documentElement.style.setProperty('--quick-days-count', $container.find('.quick-day-btn').length);
            let selectedDay = null;
            if (originalDate && originalDate.trim() !== '') {
                const originalFormattedDate = that.getDateAsIso(originalDate);
                daysToShow.forEach(day => {
                    if (day.date === dayjs(originalFormattedDate).format('YYYY-MM-DD')) {
                        selectedDay = day;
                    }
                }
                );
            }
            if (!selectedDay && daysToShow.length > 0) {
                selectedDay = daysToShow[0];
            }
            if (!selectedDateInCalendarByUser && selectedDay || selectedDateInCalendarByUser == selectedDay.date) {
                $container.find(`.quick-day-btn[data-date="${selectedDay.date}"]`).addClass('active');
                const formattedDate = dayjs(selectedDay.date).format(that.calendarSettings.format);
                that.$hiddenInput.val(formattedDate);
                that.$fakeInput.html('<div class="date-value">' + formattedDate + '</div><div class="day-name">(' + that.getDateDayName(selectedDay.date) + ')</div>');
                that.HoursInput.init({
                    ...hoursInputOriginalSettings,
                    isQuickDaysValidator: false
                });
    ...
    that.setDate = function(date) {
        var dayObj = dayjs(date);
        dateTXT = dayObj.format(that.calendarSettings.format);
        that.$fakeInput.html('<div class="date-value">' + dateTXT + '</div><div class="day-name">(' + that.getDateDayName(date) + ')</div>');
        that.$hiddenInput.val(dateTXT);
    }
    ;
    that.addPluginAbilityToInput = function() {
    ...
    that.showPopUp = function() {
        ...
        var $modal = that.calendarModalParent.bootbox.dialog({
            ...
            that.isShown = false;
            if (that.HoursInput.$tableHour.data('tomselect')) {
                that.HoursInput.$tableHour.data('tomselect').destroy();
                new UiRichSelect();
            }
        });
        $modal.find('.modal-header').addClass('background-primary-color btn-primary-text-color');
    ...
    that.loadDateAndTimeFromInput = function() {
        if (that.$hiddenInput.val().length === 0)
            return;
        that.$calendar.attr('navigate-view-only', 'true');
        that.$calendar.datepicker('setDates', new Date(that.getDateAsIso(that.$hiddenInput.val())));
    }
    ;
    that.getCalendar = function() {
        ...
        that.calendarSettings.daysOfWeekDisabled = disabledDays.join(',');
        if (that.calendarSettings.availability == 'bySchedule') {
            $calendar.on('show changeMonth changeYear changeDecade changeCentury', function(event) {
                that.calendarSettings.daysOfWeekDisabled = '';
                let $loading = $('<div class="calendar-loading-overlay text-center">' + S123.s123IconToSvg.getHtml('spinner', 'fa-spin fa-2x') + '</div>');
                $calendar.parent().append($loading);
                setTimeout(function() {
                    if (event.type != 'show') {
                        let startDate = dayjs(event.date).startOf('month');
                        let maxDate = startDate.endOf('month');
                        let lastDateOfMonth = dayjs(that.hoursInputSettings.pageLoadSettings.lastDateOfMonth);
                        if (maxDate.isAfter(lastDateOfMonth) || maxDate.isSame(lastDateOfMonth)) {
                            let serviceStaffMembers = [];
                            that.$rootContainer.closest('.s123-module-schedule-bookingV2').find('.staffMemberSettings').each(function(index, staffMemberSettings) {
                                staffMemberSettings = tryParseJSON($(staffMemberSettings).text());
                                serviceStaffMembers.push(staffMemberSettings.uniqueID);
                            });
                            let newLastDateOfMonth = startDate.add(that.schedulesOffset.maxDate, 'day').format('YYYY-MM-DD');
                            $.ajax({
                                url: '/versions/2/wizard/modules/scheduleBookingV2/get-scheduled-orders.php',
                                method: 'post',
                                ...
                                data = tryParseJSON(data);
                                that.hoursInputSettings.pageLoadSettings.orders = [...that.hoursInputSettings.pageLoadSettings.orders, ...data.orders];
                                if (data.sessions) {
                                    if ($.isArray(that.hoursInputSettings.pageLoadSettings.sessions)) {
                                        that.hoursInputSettings.pageLoadSettings.sessions = [...that.hoursInputSettings.pageLoadSettings.sessions, ...data.sessions];
                                    } else {
                                        that.hoursInputSettings.pageLoadSettings.sessions = data.sessions;
                                    }
                                }
                                if (data.resourceEvents) {
                                    if ($.isArray(that.hoursInputSettings.pageLoadSettings.resourceEvents)) {
                                        that.hoursInputSettings.pageLoadSettings.resourceEvents = [...that.hoursInputSettings.pageLoadSettings.resourceEvents, ...data.resourceEvents];
                                    } else {
                                        that.hoursInputSettings.pageLoadSettings.resourceEvents = data.resourceEvents;
                                    }
                                }
                                that.hoursInputSettings.pageLoadSettings.lastDateOfMonth = newLastDateOfMonth;
                                checkAndDisableDaysWithNoAvailableTimes($calendar, startDate, maxDate, dayjs(event.date).startOf('month'));
                                $loading.remove();
                            });
                        } else {
                            startDate = startDate.subtract(that.schedulesOffset.minDate, 'days');
                            maxDate = maxDate.add(that.schedulesOffset.minDate, 'days');
                            checkAndDisableDaysWithNoAvailableTimes($calendar, startDate, maxDate, dayjs(event.date).startOf('month'));
                            $loading.remove();
                        }
                    } else {
                        let startDate = dayjs(that.getDateAsIso(that.$hiddenInput.val())).startOf('month').subtract(that.schedulesOffset.minDate, 'days');
                        let maxDate = dayjs(that.getDateAsIso(that.$hiddenInput.val())).endOf('month').add(that.schedulesOffset.minDate, 'days');
                        checkAndDisableDaysWithNoAvailableTimes($calendar, startDate, maxDate, dayjs(that.getDateAsIso(that.$hiddenInput.val())).startOf('month'));
                        $loading.remove();
                    }
                }, 0);
            });
        ...
        $calendar.datepicker({
            format: that.calendarSettings.format,
            weekStart: that.calendarSettings.firstDayOfWeek,
            todayBtn: that.calendarSettings.todayBtn,
            clearBtn: that.calendarSettings.clearBtn,
            language: that.calendarSettings.language,
            startDate: that.calendarSettings.startDate,
    ...
    that.datePickerSubmit = function(calendar, modal) {
        calendar.off('changeDate').on('changeDate', function(ev) {
            if ($(this).attr('navigate-view-only') == 'true') {
                $(this).removeAttr('navigate-view-only');
                return;
            }
            if (!that.onSubmit)
                return;
            ...
            that.onSubmit.call(modal, formattedSelectedDate, selectedDate);
            if (that.hoursInputSettings) {
                that.$hiddenInput.val(formattedSelectedDate);
                that.HoursInput.init({
                    ...that.hoursInputOriginalSettings,
                    isQuickDaysValidator: false
                });
            ...
            if (that.hoursInputSettings) {
                let $loading = $('<div class="calendar-loading-overlay text-center">' + S123.s123IconToSvg.getHtml('spinner', 'fa-spin fa-2x') + '</div>');
                that.$calendar.parent().append($loading);
                setTimeout(function() {
                    initializeQuickAvailableDays({
                        calendarSettings: that.calendarSettings,
                        hoursInputSettings: that.hoursInputSettings,
                        $moduleSelector: that.$moduleSelector,
                        moduleID: that.moduleID,
    ...
    ;
    that.isUnAvalible = function() {
        return that.inActiveDays == 7 || that.calendarSettings.availability == 'unavailable' || that.calendarSettings.isServiceUnavailable;
    }
    ;
    that.showUnavailableMsg = function() {
        that.$rootContainer.find('.c-h-widget').addClass('hidden');
    ...
    function initializeBusinessHours() {
        that.inActiveDays = 0;
        if (that.calendarSettings.availability == 'byStaff') {
            $.each(that.calendarSettings.businessHours, function(index, dayOfWeek) {
                if (!dayOfWeek.isActive) {
                    that.inActiveDays++;
                }
        ...
        if (['fullTime', 'bySchedule'].includes(that.calendarSettings.availability)) {
            $.each(that.calendarSettings.businessHours, function(index, weekday) {
                weekday.isActive = true;
                for (let i = 0; i <= that.maxShiftsAmount; i++) {
                    if (i == 1) {
                        weekday[`startTime${i}`] = '00:00';
                        weekday[`endTime${i}`] = '24:00';
                    } else {
                        weekday[`startTime${i}`] = '';
                        weekday[`endTime${i}`] = '';
                    }
    ...
    function checkAndDisableDaysWithNoAvailableTimes($calendar, baseDate, maxDateToCheck, calendarFirstDayOfMonth) {
        let today = dayjs();
        if (today.isAfter(baseDate)) {
            baseDate = today;
        }
        const originalValue = that.$hiddenInput.val();
        let dayIndex = 0;
        const datesToDisable = {};
        const activeDates = {};
        const hoursInputSettings = {
            calendarSettings: that.calendarSettings,
            hoursInputSettings: {
                ...that.hoursInputSettings,
                onHoursLoad: function(chWidget, $tableHour) {
                    let isDayAvailable = $tableHour.find('option:not([value=""])').length > 0;
                    const dateToCheck = that.$hiddenInput.val();
                    const isoDate = that.getDateAsIso(dateToCheck);
                    const dateObj = new Date(isoDate);
                    if (that.calendarSettings.availability != 'bySchedule') {
                        if (!that.calendarSettings.businessHours[dayjs(isoDate).day()].isActive) {
                            isDayAvailable = false;
                        }
                    }
                    if (!isDayAvailable) {
                        if (!datesToDisable[isoDate]) {
                            datesToDisable[isoDate] = dateObj;
                        }
                    } else {
                        if (!activeDates[isoDate]) {
                            activeDates[isoDate] = dateObj;
                        }
                    }
                    dayIndex++;
                    let nextDay = baseDate.add(dayIndex, 'day');
                    if (!nextDay.isAfter(maxDateToCheck)) {
                        checkDay(nextDay);
                    }
                    if (that.originalOnHoursLoad)
                        that.originalOnHoursLoad.call(this, that, $tableHour);
                }
            },
            $moduleSelector: that.$moduleSelector,
            moduleID: that.moduleID,
        ...
        if (Object.keys(datesToDisable).length > 0) {
            $calendar.datepicker('setDatesDisabled', Object.values(datesToDisable));
            $.each(activeDates, function(key, value) {
                if (dayjs(key).isBefore(calendarFirstDayOfMonth)) {
                    delete activeDates[key];
                }
            });
            $calendar.attr('navigate-view-only', 'true');
            if (Object.values(activeDates).length > 0) {
                $calendar.datepicker('setDate', new Date(dayjs(Object.values(activeDates)[0]).format('YYYY-MM-DD')));
            } else {
                $calendar.datepicker('setDate', new Date(calendarFirstDayOfMonth.format('YYYY-MM-DD')));
            }
        }
        that.$hiddenInput.val(originalValue);
        that.HoursInput.init({
            ...that.hoursInputOriginalSettings,
            isQuickDaysValidator: true
        });
        function checkDay(dateToCheck) {
            that.$hiddenInput.val(dateToCheck.format(that.calendarSettings.format));
            that.HoursInput.init({
                ...hoursInputSettings,
                isQuickDaysValidator: true
            });
    ...
    that.HoursInput = function() {
        ...
        _.init = function(settings) {
            ...
            _.maxParticipants = settings.hoursInputSettings.maxParticipants;
            _.timeInterval = settings.hoursInputSettings.timeInterval;
            _.pageLoadSettings = settings.hoursInputSettings.pageLoadSettings;
            _.$moduleSelector = settings.$moduleSelector;
            _.isQuickDaysValidator = settings.isQuickDaysValidator;
            _.$rootContainer = settings.$rootContainer;
            _.serviceDuration = settings.serviceDuration;
            _.timeBetweenService = settings.timeBetweenService;
            _.$serviceDatesContainer = settings.$serviceDatesContainer;
            _.$serviceHourContainer = settings.$serviceHourContainer;
            _.$tableHour = settings.$tableHour;
            _.$input = settings.$input;
            _.isPageLoad = true;
        ...
        function refreshBusinessHours() {
            ...
            selectedDate = changeDateFormat('YYYY-mm-DD ' + _.calendarSettings.originalFormat, selectedDate);
            var today = new Date(selectedDate);
            var lastDateOfMonth = new Date(today);
            lastDateOfMonth.setDate(lastDateOfMonth.getDate() + that.schedulesOffset.maxDate);
            lastDateOfMonth.setHours(23, 59, 59);
            lastDateOfMonth = getDateFormat(lastDateOfMonth) + ' ' + getHourFromDate(lastDateOfMonth);
            if (_.$tableHour.data('tomselect')) {
                _.$tableHour.data('tomselect').destroy();
            }
            _.$tableHour.empty();
            showHoursLoading();
            let serviceStaffMembers = [];
            _.$rootContainer.closest('.s123-module-schedule-bookingV2').find('.staffMemberSettings').each(function(index, staffMemberSettings) {
                staffMemberSettings = tryParseJSON($(staffMemberSettings).text());
                serviceStaffMembers.push(staffMemberSettings.uniqueID);
            });
            if (_.isPageLoad && _.pageLoadSettings) {
                ordersLoadedCallback(_.pageLoadSettings);
                _.isPageLoad = false;
            } else {
                $.ajax({
                    url: '/versions/2/wizard/modules/scheduleBookingV2/get-scheduled-orders.php',
                    ...
                    data = tryParseJSON(data);
                    ordersLoadedCallback(data);
                }).fail(function() {
                    hideHoursLoading();
                });
            }
            function ordersLoadedCallback(data) {
                _.$serviceDatesContainer.data('corrent-time', data.userTime);
                if (_.calendarSettings.availability == 'bySchedule') {
                    buildSessionsHourSelectBox(data.sessions);
                } else {
                    buildHourSelectBox(data.orders);
                }
                removeUnavailableHours(data.orders, data.resourceEvents);
                if (!_.isQuickDaysValidator) {
                    if (_.$tableHour.find('option').length <= 0) {
                        _.$tableHour.addClass('hidden');
                        _.$serviceHourContainer.find('.no-time-available').removeClass('hidden');
                        _.$serviceHourContainer.find('.quick-time-slots-container').remove();
                    } else {
                        _.$tableHour.removeClass('hidden');
                        _.$serviceHourContainer.find('.no-time-available').addClass('hidden');
                        _.$tableHour.prepend('<option value="" selected>' + translations.chooseTime + '</option>');
                        new UiRichSelect();
                ...
                if (_.onHoursLoad)
                    _.onHoursLoad.call(this, that, _.$tableHour);
                if (_.isPageLoad) {
                    setTimeout( () => {
                        hideHoursLoading();
                    }
            ...
            function removeUnavailableHours(unavailableHours, resourceEvents) {
                var selectedDate = _.$input.val();
                selectedDate = changeDateFormat('YYYY-mm-DD ' + _.calendarSettings.originalFormat, selectedDate);
                var serviceDurationHour = _.serviceDuration.substring(0, 2);
                var serviceDurationMin = _.serviceDuration.substring(3, 5);
                ...
                var serviceDurationInMili = serviceDurationHourMili + serviceDurationMinutesMili + timeBetweenService;
                let timesToRemove = {};
                if (['byStaffDateFirst', 'autoStaffByDate'].includes(that.calendarSettings.staffOrderFlow)) {
                    fillTimesToRemove(that.$moduleSelector.find('.staff-members-container .staff-member-card'));
                } else {
                    fillTimesToRemove(that.$moduleSelector.find('.staff-members-container .staff-member-card[data-item-tool-staff-id="' + _.$form.find('[name="item_tool_staff_id"]').val() + '"]'));
                }
                removeOptions('businessHours');
                if (S123.QueryString.reschedule == 1 && selectedDate == dayjs(S123.QueryString.currentDate).format('YYYY-MM-DD')) {
                    timesToRemove[dayjs(S123.QueryString.currentDate).format('HH:mm')] = {
                        time: dayjs(S123.QueryString.currentDate).format('HH:mm'),
                        count: 1,
                        isForceRemove: true
                    };
                    removeOptions('reschedule');
                }
                if (_.calendarSettings.availability != 'bySchedule') {
                    if (that.calendarSettings.resourceTypes && Array.isArray(that.calendarSettings.resourceTypes)) {
                        if (resourceEvents && Array.isArray(resourceEvents) && resourceEvents.length > 0) {
                            _.$tableHour.find('option').each(function() {
                                let option = $(this);
                                let timeValue = option.val();
                                if (!timeValue)
                                    return;
                                let optionTime = dayjs(selectedDate + ' ' + timeValue);
                                let optionEnd = optionTime.add(serviceDurationInMili, 'millisecond');
                                let resourceUsage = {};
                                that.calendarSettings.resourceTypes.forEach(resourceType => {
                                    resourceUsage[resourceType] = 0;
                                }
                                );
                                resourceEvents.forEach( (event) => {
                                    let eventStart = dayjs(event.startDateFUN);
                                    let eventEnd = dayjs(event.endDateFUN);
                                    if (isOptionIntersectinWithService({
                                        start: optionTime.valueOf(),
                                        end: optionEnd.valueOf()
                                    }, {
                                        start: eventStart.valueOf(),
                                        end: eventEnd.valueOf(),
                                        obj: event
                                    })) {
                                        if (event.resources && Array.isArray(event.resources)) {
                                            event.resources.forEach(resource => {
                                                const totalItems = parseInt(resource.totalItems) || 0;
                                                if (totalItems == 0)
                                                    return;
                                                let isRemoveOption = false;
                                                if (!resourceUsage[resource.resourceTypeID]) {
                                                    resourceUsage[resource.resourceTypeID] = 0;
                                                }
                                                const resourcesInUse = parseInt(resource.resourcesInUse) || 0;
                                                resourceUsage[resource.resourceTypeID] += resourcesInUse;
                                                isRemoveOption = resourceUsage[resource.resourceTypeID] >= totalItems;
                                                if (isRemoveOption && option.data('multiple-participants-resources-handler')) {
                                                    isRemoveOption = false;
                                                    let availableStaff = option.data('available-staff');
                                                    $.each(availableStaff, function(index, staff) {
                                                        let isRemoveStaff = false;
                                                        if (option.data('multiple-participants-resources-handler').staffCount[staff.id] >= _.maxParticipants) {
                                                            isRemoveStaff = true;
                                                        }
                                                        if (!option.data('multiple-participants-resources-handler').staffCount[staff.id]) {
                                                            isRemoveStaff = true;
                                                        }
                                                        if (isRemoveStaff) {
                                                            availableStaff = availableStaff.filter(availableStaff => availableStaff.id != staff.id);
                                                            option.data('available-staff', availableStaff);
                                                            option.attr('data-available-staff', JSON.stringify(availableStaff));
                                                        }
                                                    });
                            ...
                            removeOptions('resources');
                        }
                    }
                }
                function fillTimesToRemove($staffMembers) {
                    let staffMembers = $staffMembers.map(function() {
                        return {
                            id: $(this).data('item-tool-staff-id'),
                            fname: $(this).find('.staff-member-name').text(),
                            businessHours: StaffMembersHelpers.getStaffSettings(that.$moduleSelector, $(this).data('item-tool-staff-id'), 'businessHours'),
                            unavailableHours: unavailableHours.filter( (unavailableHour) => {
                                return unavailableHour.item_tool_staff_id == $(this).data('item-tool-staff-id');
                            }
                            )
                        };
                    }).get();
                    _.$tableHour.find('option').each(function() {
                        ...
                        if (!timeValue)
                            return;
                        let optionTimeStart = dayjs(selectedDate + ' ' + timeValue);
                        let dayOfWeek = optionTimeStart.day();
                        let availableStaff = [];
                        staffMembers.forEach( (staff) => {
                            let staffObject = {
                                id: staff.id,
                                fname: staff.fname,
                                remainingCapacity: _.maxParticipants
                            };
                            if (that.calendarSettings.availability != 'bySchedule') {
                                if (!staff.businessHours[dayOfWeek].isActive || !isWithinBusinessHours(optionTimeStart, staff.businessHours[dayOfWeek])) {
                                    return;
                                }
                            }
                            if (that.calendarSettings.availability == 'bySchedule' && !option.data('related-schedules').find( (relatedSchedule) => {
                                return relatedSchedule.item_tool_staff_id == staffObject.id;
                            }
                            ))
                                return;
                            if (staff.unavailableHours.length == 0) {
                                availableStaff.push(staffObject);
                                return;
                            }
                            let isStaffMemberAvailable = true;
                            let timeMaxParticipants = _.maxParticipants;
                            $.each(staff.unavailableHours.filter( (unavailableHour) => {
                                return dayjs(unavailableHour.orderDate).valueOf() == optionTimeStart;
                            }
                            ), (index, unavailableHour) => {
                                let orderDate = dayjs(unavailableHour.orderDate).valueOf();
                                if (orderDate != optionTimeStart) {
                                    return;
                                } else if (orderDate == optionTimeStart && that.itemID != unavailableHour.itemID) {
                                    timeMaxParticipants = 0;
                                    return;
                                }
                                timeMaxParticipants--;
                            }
                            );
                            if (timeMaxParticipants <= 0) {
                                isStaffMemberAvailable = false;
                            }
                            $.each(staff.unavailableHours.filter( (unavailableHour) => {
                                return dayjs(unavailableHour.orderDate).valueOf() != optionTimeStart;
                            }
                                ...
                                let orderDate = dayjs(unavailableHour.orderDate).valueOf();
                                let orderDuration = dayjs(unavailableHour.endOrderDate).valueOf() - orderDate + timeBetweenService;
                                if (selectedDate == dayjs(unavailableHour.orderDate).format('YYYY-MM-DD') && unavailableHour.isAllDayEvent) {
                                    isStaffMemberAvailable = false;
                                    return false;
                                }
                                if (isOptionIntersectinWithService({
                                    start: optionTimeStart,
                                    end: optionTimeStart + serviceDurationInMili
                                }, {
                                    start: orderDate,
                                    end: orderDate + orderDuration,
                                    obj: unavailableHour
                                })) {
                                    isStaffMemberAvailable = false;
                                    return false;
                        ...
                        if (availableStaff.length > 0) {
                            ...
                            if (!timesToRemove[timeValue]) {
                                timesToRemove[timeValue] = {
                                    time: timeValue,
                                    count: unavailableHours.length,
                                    obj: [...unavailableHours],
                                    isForceRemove: true // set to maximum participants so block engine will remove the time
                                };
                            }
                        }
                    });
                }
                function removeOptions(validatorType) {
                    if (Object.keys(timesToRemove).length > 0) {
                        _.$tableHour.find('option').each(function() {
                            let $option = $(this);
                            let timeValue = $option.val();
                            if (!timesToRemove[timeValue])
                                return;
                            if (timesToRemove[timeValue].count >= _.maxParticipants) {
                                $option.remove();
                            } else if (timesToRemove[timeValue].isForceRemove) {
                                $option.remove();
                            }
                        });
                        timesToRemove = {};
                ...
                function isWithinBusinessHours(time, businessHours) {
                    for (let i = 0; i <= that.maxShiftsAmount; i++) {
                        let start = businessHours[`startTime${i}`];
                        let end = businessHours[`endTime${i}`];
                        if (start && end) {
                            let startTime = dayjs(time.format('YYYY-MM-DD') + ' ' + start);
                            let endTime = dayjs(time.format('YYYY-MM-DD') + ' ' + end);
                            if (time.valueOf() >= startTime.valueOf() && time.add(serviceDurationInMili, 'millisecond').valueOf() <= endTime.valueOf()) {
                                return true;
                            }
                        }
                    }
                ...
                function isOptionIntersectinWithService(option, order) {
                    if (option.start >= order.start && option.start < order.end) {
                        return true;
                    } else if (option.end > order.start && option.end <= order.end) {
                        return true;
                    } else if (option.start < order.start && option.end > order.end) {
                        return true;
                    } else if (option.start >= order.start && option.end <= order.end) {
                        return true;
                    } else if (option.start === order.start && option.end === order.end) {
                    ...
                    if (order.obj.isAllDayEvent && selectedDate == dayjs(order.start).format('YYYY-MM-DD')) {
                        return true;
                    }
                    return false;
                }
            ...
            function buildHourSelectBox(unavailableHours) {
                var serviceDurationHour = _.serviceDuration.substring(0, 2);
                var serviceDurationHourMili = parseInt(serviceDurationHour) * 60 * 60 * 1000;
                var serviceDurationMin = _.serviceDuration.substring(3, 5);
                var serviceDurationMinutesMili = parseInt(serviceDurationMin) * 60 * 1000;
                ...
                if (!dateFromCalendar)
                    throw 'Missing date parameter';
                let multipleParticipantsResourcesDateHandler = {};
                if (_.maxParticipants > 1) {
                    $.each(unavailableHours, function(index, order) {
                        let orderHour = dayjs(order.orderDate).format('HH:mm');
                        if (dayjs(order.orderDate).format('YYYY-MM-DD') == dateFromCalendar) {
                            if (!multipleParticipantsResourcesDateHandler[orderHour]) {
                                multipleParticipantsResourcesDateHandler[orderHour] = {
                                    staffCount: {}
                                };
                            }
                            if (!multipleParticipantsResourcesDateHandler[orderHour].staffCount[order.item_tool_staff_id]) {
                                multipleParticipantsResourcesDateHandler[orderHour].staffCount[order.item_tool_staff_id] = 0;
                            }
                            multipleParticipantsResourcesDateHandler[orderHour].staffCount[order.item_tool_staff_id]++;
                        }
                    });
                }
                var dataIndex = dayjs(dateFromCalendar).day();
                if (that.staffAvailability) {
                    that.staffAvailability.clear();
                }
                let allStaffIds = [];
                if (['byStaffDateFirst', 'autoStaffByDate'].includes(that.calendarSettings.staffOrderFlow) && that.$moduleSelector) {
                    allStaffIds = that.$moduleSelector.find('.staff-members-container').find('.staff-member-card').map(function() {
                        return {
                            id: $(this).data('item-tool-staff-id'),
                            businessHours: StaffMembersHelpers.getStaffSettings(that.$moduleSelector, $(this).data('item-tool-staff-id'), 'businessHours')
                        };
                ...
                for (var i = 0; i <= that.maxShiftsAmount; i++) {
                    ...
                    endTime = dayjs(dateFromCalendar + " " + endTime).valueOf();
                    for (; startTime <= (endTime - serviceDurationHourMili - serviceDurationMinutesMili); startTime += serviceInterval) {
                        var newdate = new Date(startTime);
                        var newHour = newdate.getHours();
                        var newMinutes = newdate.getMinutes();
                        if (newHour.toString().length == 1)
                        ...
                        $.each(_.$tableHour.find('option'), function(index) {
                            ...
                            $(this).data('multiple-participants-resources-handler', multipleParticipantsResourcesDateHandler[$optionVal]);
                            $(this).attr('data-multiple-participants-resources-handler', JSON.stringify(multipleParticipantsResourcesDateHandler[$optionVal]));
                            if ($optionVal == fullHour) {
                                hourExists = true;
                                return false;
                            }
                        ...
                        if (['byStaffDateFirst', 'autoStaffByDate'].includes(that.calendarSettings.staffOrderFlow) && allStaffIds.length > 0) {
                            staffAvailable = allStaffIds.some(staff => {
                                if (!staff.businessHours[dataIndex].isActive) {
                                    return false;
                                }
                                const timeObj = dayjs(dateFromCalendar + ' ' + fullHour);
                                for (let j = 0; j < that.maxShiftsAmount; j++) {
                                    const shiftStart = staff.businessHours[dataIndex][`startTime${j}`];
                                    const shiftEnd = staff.businessHours[dataIndex][`endTime${j}`];
                                    if (shiftStart && shiftEnd) {
                                        const shiftStartMs = dayjs(dateFromCalendar + ' ' + shiftStart).valueOf();
                                        const shiftEndMs = dayjs(dateFromCalendar + ' ' + shiftEnd).valueOf();
                                        const timeMs = timeObj.valueOf();
                                        const endTimeMs = timeMs + serviceDurationHourMili + serviceDurationMinutesMili;
                                        if (timeMs >= shiftStartMs && endTimeMs <= shiftEndMs) {
                                            return true;
                                        }
                                    }
                                }
                        ...
                        if (!hourExists && staffAvailable) {
                            let $option = $('<option value="' + newHour + ':' + newMinutes + '">' + changeTimeFormat(_.calendarSettings.timeFormat, newdate) + '</option>');
                            $option.data('multiple-participants-resources-handler', multipleParticipantsResourcesDateHandler[newHour + ':' + newMinutes]);
                            $option.attr('data-multiple-participants-resources-handler', JSON.stringify(multipleParticipantsResourcesDateHandler[newHour + ':' + newMinutes]));
                            _.$tableHour.append($option);
                        }
                    }
                }
                function changeTimeFormat(websiteTimeFormat, date) {
                ...
                _.$tableHour.off('change').on('change', function(event) {
                    if (_.onHoursChage)
                        _.onHoursChage.call(this, $(this).val(), $(this).find('option:selected'));
                });
            }
            function initQuickTimeButtons() {
                _.$serviceHourContainer.find('.quick-time-slots-container').remove();
                _.$serviceHourContainer.removeClass('quick-time-mode');
                const $timeOptions = $(Object.values(_.$tableHour.data('tomselect').options).map(option => {
                    return option.$option
                }
                ));
                if ($timeOptions.length <= 15 && $timeOptions.length > 0) {
                    let $quickTimeContainer = $('<div class="quick-time-slots-container"></div>');
                    _.$serviceHourContainer.append($quickTimeContainer);
                    $timeOptions.each(function() {
                        const $option = $(this);
                        const timeValue = $option.val();
                        const displayTime = $option.text();
                        if (!timeValue || ($option.attr('data-available-staff') === '[]')) {
                            return;
                        }
                        const $button = $(`<button type="button" class="quick-time-btn" data-time="${timeValue}">${displayTime}</button>`);
                        $.each($option.data(), function(key, value) {
                            if (typeof value === 'object') {
                                $button.attr('data-' + key, JSON.stringify(value));
                            } else {
                                $button.attr('data-' + key, value);
                            }
                        });
                        $button.on('click', function() {
                            _.$tableHour.data('tomselect').setValue($(this).data('time'));
                            $quickTimeContainer.find('.quick-time-btn').removeClass('active');
                            $(this).addClass('active');
                        });
                        $quickTimeContainer.append($button);
                    });
                    _.$serviceHourContainer.addClass('quick-time-mode');
                }
            }
            function buildSessionsHourSelectBox(sessions) {
                var dateFromCalendar = changeDateFormat('YYYY-mm-DD ' + _.calendarSettings.originalFormat, _.$input.val());
                var dayOfWeek = dayjs(dateFromCalendar).day();
                let startTime = dayjs(dateFromCalendar + " " + _.$serviceDatesContainer.data('corrent-time')).valueOf();
                $.each(sessions, function(index, session) {
                    if (dayOfWeek != dayjs(session.startDateFUN).day())
                        return;
                    if (dateFromCalendar != dayjs(session.startDateFUN).format('YYYY-MM-DD'))
                        return;
                    let sessionTime = dayjs(`${session.startDateFUN}`).format('HH:mm');
                    if (dayjs().format('YYYY-MM-DD') == dayjs(`${session.startDateFUN}`).format('YYYY-MM-DD')) {
                        if (dayjs(`${session.startDateFUN}`).valueOf() < startTime)
                            return;
                    }
                    let $option = _.$tableHour.find('option[value="' + sessionTime + '"]');
                    if ($option.length > 0) {
                        $option.data('related-schedules').push({
                            eventID: session.eventID,
                            duration: session.duration,
                            item_tool_staff_id: session.item_tool_staff_id
                        });
                        $option.attr('data-related-schedules', JSON.stringify($option.data('related-schedules')));
                    } else {
                        _.$tableHour.append('<option data-duration="' + S123.escapeHtml(JSON.stringify([{
                            duration: session.duration
                        }])) + '"' + '" value="' + sessionTime + '" data-related-schedules="' + S123.escapeHtml(JSON.stringify([{
                            eventID: session.eventID,
                            duration: session.duration,
                            item_tool_staff_id: session.item_tool_staff_id
                        }])) + '" data-schedule-selected-date="' + dateFromCalendar + ' ' + sessionTime + '">' + sessionTime + '</option>');
                    }
                });
                _.$tableHour.off('change').on('change', function(event) {
                    if (_.onHoursChage)
        ...
        function showHoursLoading() {
            const $hoursContainer = that.$rootContainer.find('.serviceHourContainer');
            $hoursContainer.addClass('loading');
            if ($hoursContainer.find('.hours-loading-overlay').length === 0) {
                const $loadingOverlay = $('<div class="hours-loading-overlay box-primary">' + S123.s123IconToSvg.getHtml('spinner', 'fa-spin fa-2x') + '</div>');
                $hoursContainer.append($loadingOverlay);
            } else {
                $hoursContainer.find('.hours-loading-overlay').show();
            }
        }
        function hideHoursLoading() {
            const $hoursContainer = that.$rootContainer.find('.serviceHourContainer');
            $hoursContainer.removeClass('loading');
            const $spinner = $hoursContainer.find('.hours-loading-overlay');
            if ($spinner.length > 0) {
                $spinner.remove();
            }
        }
    ...
    var CanvasRenderer = function(el, options) {
        ...
        var drawBackground = function() {
            ...
            if (options.trackColor)
                drawCircle(options.trackColor, options.trackWidth || options.lineWidth, 1);
        };
...
var S123_ActionButtons = function() {
    var that = {
        settings: {},
        custom_form_html: {},
        moduleIDs: {},
        custom_thank_you_msg: {},
        customFromInputs: {},
    };
    that.init = function() {
        $('[data-toggle="actionButton_customForm"]').each(function() {
            $(this).off('click').on('click', function() {
                var $this = $(this);
                var uniqueID = $this.data('settings-id');
                that.moduleIDs[uniqueID] = $this.data('module-id');
                if ($('#' + uniqueID).length == 0)
                    return;
                that.settings[uniqueID] = tryParseJSON($('#' + uniqueID).val());
                that.custom_form_html[uniqueID] = that.settings[uniqueID].custom_form_html ? that.settings[uniqueID].custom_form_html : '';
                that.custom_thank_you_msg[uniqueID] = that.settings[uniqueID].thankYouMsg ? that.settings[uniqueID].thankYouMsg : '';
                buildPopup('popupFloatDivActionButtonCustomForm', '', that.buildEmailToolForm(uniqueID), '', true, true, true, 'right', '');
                S123_ActionButtons.submitHandler(uniqueID);
            });
        });
    }
    ;
    that.buildEmailToolForm = function(uniqueID) {
        var html = '';
        html += '<div class="global-contact-email-container">';
        that.customFromInputs[uniqueID] = $(that.custom_form_html[uniqueID]).find('.c-f-field-type').length;
        if (that.settings[uniqueID].showCustomForm == 'on' && that.custom_form_html[uniqueID].length !== 0 && that.customFromInputs[uniqueID] > 0) {
            html += '<form class="a-b-email-form s123-custom-form-multi-steps custom-form">';
            html += that.custom_form_html[uniqueID];
            ...
            html += '<input type="hidden" name="moduleID" value="' + that.moduleIDs[uniqueID] + '">';
            html += '<input type="hidden" name="actionButtonForm" value="1">';
            html += '<input type="hidden" name="buttonNum" value="' + that.settings[uniqueID].buttonNum + '">';
            html += '<div class="conv-code-container"></div>';
            html += '</form>';
        } else {
            html += '<form class="a-b-email-form">';
            html += '<div class="row">';
            html += '<div class="col-sm-6 col-xs-12">';
            html += '<div class="form-group">';
            html += '<label for="contact_name" class="white">' + translations.globalContactEmail.fullName + '</label>';
            html += '<input type="text" name="contact_name" placeholder="' + translations.globalContactEmail.fullName + '" class="form-control" required data-msg-required="' + translations.jqueryValidMsgRequire + '">';
            html += '</div>';
            html += '</div>';
            html += '<div class="col-sm-6 col-xs-12">';
            html += '<div class="form-group">';
            html += '<label for="contact_email" class="white">' + translations.emailAddress + '</label>';
            html += '<input type="text" name="contact_email" placeholder="' + translations.emailAddress + '" class="form-control" required data-msg-required="' + translations.jqueryValidMsgRequire + '" data-rule-email="true" data-msg-email="' + translations.jqueryValidMsgEmail + '">';
            html += '</div>';
            html += '</div>';
            html += '</div>';
            html += '<div class="row">';
            ...
            html += '<div class="form-group">';
            html += '<label for="contact_message" class="white">' + translations.globalContactEmail.description + '</label>';
            html += '<textarea class="form-control" name="contact_message" rows="4" placeholder="' + translations.globalContactEmail.description + '"></textarea>';
            html += '</div>';
            html += '</div>';
            html += '</div>';
            html += '<div class="row">';
        ...
        html += '<h3 class="g-c-email-message-content">' + (that.custom_thank_you_msg[uniqueID].length > 0 ? that.custom_thank_you_msg[uniqueID] : translations.globalContactEmail.thankYouMessage) + '</h3>';
        html += '</div>';
        html += '</div>';
        html += '<div class="row">';
        html += '<div class="col-sm-6 col-xs-12 col-md-offset-3">';
    ...
    ;
    that.submitHandler = function(uniqueID) {
        var $form = $('.a-b-email-form');
        ...
        $form.find('.f-b-date-timePicker').each(function() {
            ...
            formBuilderCalendar.init({
                ...
                customClass: 'action-button-custom-form',
                calendarSettings: {
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.action.buttonClickAction && data.action.buttonClickAction == 'redirect') {
                            if (data.action.url.length > 0) {
                                if (IsWizard()) {
                                    buildPopup_CloseAction('popupFloatDivActionButtonCustomForm');
                                    bootbox.alert({
                                        title: translations.previewExternalLinkTitle,
                                        message: translations.previewExternalLinkMsg.replace('{{externalLink}}', '<b>' + data.action.url + '</b>'),
