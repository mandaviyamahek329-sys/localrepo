var FrontendPopups = function() {
    ...
    _.open = function(settings) {
        const existingPopup = document.querySelector('.fePopupWindow');
        $('body').addClass('fePopupWindowOpen');
        setRedirectURL(settings.redirectURL);
        setIframeURL(settings.iframeURL);
        if (!existingPopup) {
            buildPopup(settings);
        }
        ...
        const popup = document.querySelector('.fePopupWindow');
        popup.style.top = '-200vh';
        popup.offsetHeight;
        setTimeout(function() {
            popup.style.top = '0';
        }, 10);
        // 10ms is enough to trigger the transition
        return popup.id;
    }
    ;
    _.close = function(refresh=false, generatedRedirectURL=null) {
        $('body').removeClass('fePopupWindowOpen');
        const redirectURL = _.redirectURL;
        if (generatedRedirectURL) {
            window.top.location.href = generatedRedirectURL;
            return;
        }
        if (redirectURL) {
    ...
    function buildPopup(settings) {
        const popID = settings.popID || uniqid('fePopupWindow');
        const $html = $(`
...
`);
        $('body').append($html);
        $('.close-fe-popup-btn').click(function() {
            FrontendPopups.close();
        });
        if (settings.iframeURL) {
            $html.find('.popupIframe').on('load', function() {
                $('.fePopupLoading').hide();
                const iframe = this;
                ...
                try {
                    ...
                    iframeWindow.addEventListener('resize', debouncedSendHeight);
                    $html.on('remove', function() {
                        observer.disconnect();
                        iframeWindow.removeEventListener('resize', debouncedSendHeight);
                        clearTimeout(resizeTimeout);
                    });
                } catch (e) {
...
(function() {
    var socialLoginPopup = null;
    const isEmbedded = window.self !== window.top;
    window.addEventListener('message', function(event) {
        if (event.data && event.data.type === 'OPEN_SOCIAL_LOGIN_POPUP') {
            if (isEmbedded) {
                try {
                    window.top.postMessage({
                        type: 'OPEN_SOCIAL_LOGIN_POPUP_EMBEDDED',
                        provider: event.data.provider,
                        redirectUrl: event.data.redirectUrl
                    }, '*');
                } catch (e) {
                    console.warn('Cannot communicate with top window for social login, using fallback');
                    window.open(event.data.redirectUrl + (event.data.redirectUrl.indexOf('?') === -1 ? '?' : '&') + 'popup=1', 'SocialLoginPopup', 'width=600,height=700,menubar=no,toolbar=no,location=no,status=no,resizable=yes,scrollbars=yes');
                }
            } else {
                let url = event.data.redirectUrl;
                url += (url.indexOf('?') === -1 ? '?' : '&') + 'popup=1';
                socialLoginPopup = window.open(url, 'SocialLoginPopup', 'width=600,height=700,menubar=no,toolbar=no,location=no,status=no,resizable=yes,scrollbars=yes');
            }
        }
        if (event.data && event.data.type === 'SOCIAL_LOGIN_RESULT') {
            if (event.data.success) {
                if (isEmbedded) {
                    try {
                        window.parent.postMessage({
                            type: 'SOCIAL_LOGIN_RESULT_EMBEDDED',
                            success: event.data.success,
                            redirectUrl: event.data.redirectUrl
                        }, '*');
                    } catch (e) {
                        console.warn('Cannot forward social login result to parent iframe');
                    }
                } else {
                    const popup = document.querySelector('.fePopupWindow');
                    const iframe = popup.querySelector('.popupIframe');
                    ...
                    if (iframe) {
                        iframe.onload = function() {
                            FrontendPopups.close(true);
                        }
                        ;
                        iframe.style.display = 'none';
                        iframe.src = redirectUrl;
                        const loading = document.createElement('div');
                        loading.className = 'fePopupLoading';
                        loading.innerHTML = S123.s123IconToSvg.getHtml('spinner', 'ace-icon fa-spin popup-loading-elements fa-5x', 'background-color: var(--inside_modules_color_text);');
                        popup.querySelector('.fePopupWindow-body').appendChild(loading);
                    }
                }
            }
        }
        if (event.data && event.data.type === 'SOCIAL_LOGIN_RESULT_EMBEDDED') {
            if (event.data.success && isEmbedded) {
                const popup = document.querySelector('.fePopupWindow');
                if (popup) {
                    const iframe = popup.querySelector('.popupIframe');
                    const redirectUrl = event.data.redirectUrl;
...
function CarouselModuleInitialize() {
    $(document).on('s123.page.ready', function(event) {
        ...
        $sections.each(function() {
            var $section = $(this);
            var $layout7 = $section.find('.layout-7');
            if ($layout7.length > 0) {
                var $carouselGrid = $layout7.find('.carousel-grid');
                if ($carouselGrid.length > 0) {
                    $(document).on('keydown', function(e) {
                        if (e.key === ' ' || e.key === 'Spacebar') {
                            // Spacebar
                            e.preventDefault();
                            var $activeCarousel = $('.s123-module-carousel.layout-7 .carousel-grid');
                            if ($activeCarousel.length > 0) {
                                if ($activeCarousel.css('animation-play-state') === 'paused') {
                                    $activeCarousel.css('animation-play-state', 'running');
                                } else {
                                    $activeCarousel.css('animation-play-state', 'paused');
                                }
                            }
                        }
                    });
                    var touchStartY = 0;
                    var touchEndY = 0;
                    $layout7.on('touchstart', function(e) {
                        touchStartY = e.originalEvent.touches[0].clientY;
                    });
                    $layout7.on('touchend', function(e) {
                        touchEndY = e.originalEvent.changedTouches[0].clientY;
                        var touchDiff = Math.abs(touchStartY - touchEndY);
                        if (touchDiff > 10) {
                            var $carouselGrid = $(this).find('.carousel-grid');
                            $carouselGrid.css('animation-play-state', 'paused');
                            setTimeout(function() {
                                $carouselGrid.css('animation-play-state', 'running');
                            }, 2000);
                        }
                    });
                }
            ...
            $sectionThis.find('.contactUsForm').each(function(index) {
                ...
                $form.validate({
                    ...
                    submitHandler: function(form) {
                        ...
                        var url = "/versions/2/include/contactO.php";
                            ...
                            success: function(data) {
                                ...
                                if (clickAction == 'thankYouMessage' || clickAction == '') {
                                    bootbox.alert({
                                        ...
                                        message: thankYouMessage + '<iframe src="/versions/2/include/contactSentO.php?w=' + $('#w').val() + '&websiteID=' + dataObj.websiteID + '&moduleID=' + dataObj.moduleID + '" style="width:100%;height:30px;" frameborder="0"></iframe>',
                        ...
                        $.ajax({
                            ...
                            success: function(data) {
                                ...
                                if (clickAction == 'thankYouMessage' || clickAction == '') {
                                    bootbox.alert({
                                        title: translations.sent,
                                        message: translations.ThankYouAfterSubmmit + '<iframe src="/versions/2/include/contactSentO.php?w=' + $('#w').val() + '&websiteID=' + dataObj.websiteID + '&moduleID=' + dataObj.moduleID + '" style="width:100%;height:30px;" frameborder="0"></iframe>',
                                        className: 'contactUsConfirm',
                                        buttons: {
                                            ok: {
                                                label: translations.Ok
...
function CountdownWidget() {
    ...
    CW.generateHTML = function() {
        ...
        html += '<h4 class="countdown-digits box-text-primary"></h4>';
        html += '<p class="section_small_text countdown-label">' + CW.translate.days + '</p>';
        html += '</div>';
        html += '<div class="countdown-item hours box-primary">';
        html += '<h4 class="countdown-digits box-text-primary"></h4>';
        html += '<p class="section_small_text countdown-label">' + CW.translate.hours + '</p>';
        html += '</div>';
        html += '<div class="countdown-item minutes box-primary">';
        html += '<h4 class="countdown-digits box-text-primary"></h4>';
        html += '<p class="section_small_text countdown-label">' + CW.translate.minutes + '</p>';
        html += '</div>';
        html += '<div class="countdown-item seconds box-primary">';
        html += '<h4 class="countdown-digits box-text-primary"></h4>';
        html += '<p class="section_small_text countdown-label">' + CW.translate.seconds + '</p>';
        html += '</div>';
        html += '</div>';
        var $html = $(html);
        if (CW.type == '2') {
    ...
    CW.updateClock = function() {
        ...
        $.each(timer.parts, function(index, value) {
            if (CW.items[index].length)
                CW.items[index].html(value);
        });
        if (timer.total <= 0) {
            if (CW.onStop)
                CW.onStop();
...
function getBarColorFromClasses($section, $percentage) {
    var isPrimary = $section.hasClass('bg-primary');
    var result;
    var hasBox = $percentage.hasClass('s123-box-clean');
    var $elementToCheck = hasBox ? $section : $percentage;
    if ($elementToCheck.hasClass('bg-primary-black')) {
        result = '#ffffff';
        // White bar on black background
    } else if ($elementToCheck.hasClass('bg-primary-white')) {
        result = '#000000';
        // Black bar on white background
    } else if ($elementToCheck.hasClass('bg-primary-gray')) {
        result = '#444950';
        // Gray bar on gray background
    } else if ($elementToCheck.hasClass('background-primary-color')) {
        result = getComputedStyle(document.documentElement).getPropertyValue('--modules_color_section_btn_text').trim();
    } else {
        result = isPrimary ? getComputedStyle(document.documentElement).getPropertyValue('--modules_color_section_main').trim() : getComputedStyle(document.documentElement).getPropertyValue('--modules_color_second_section_main').trim();
    }
    return result;
}
function PercentageModuleInitialize_Layout1() {
    $(document).on('s123.page.ready', function(event) {
        $('.s123-module-percentage.layout-1, .s123-module-percentage.layout-3').each(function() {
            var $this = $(this);
            $this.find('.easy-pie-chart').each(function() {
                $(this).appear(function() {
                    var $chart = $(this);
                    var $percentage = $chart.closest('.percentage');
                    var chartSize = 115;
                    if ($this.hasClass('layout-3')) {
                        chartSize = 200;
                    }
                    $chart.attr('data-is-visible', true);
                    if (parseFloat($chart.data('percent')) <= 0) {
                        $chart.css({
                            width: chartSize,
                            height: chartSize
                        });
                    } else {
                        var barColor = $chart.data('custom-bar-color');
                        var isDefaultColor = $chart.data('is-default-color') == '1';
                        if (isDefaultColor) {
                            barColor = getBarColorFromClasses($this, $percentage);
                        }
                        $chart.easyPieChart({
                            barColor: barColor,
                            trackColor: false,
                        ...
                        $chart.on('moduleInlineEditing.saved', function() {
                            var newAmount = parseFloat($(this).text().trim());
                            $(this).attr('data-percent', newAmount);
                            $(this).data('easyPieChart').update(newAmount);
                        });
                    }
                });
            });
    ...
    if (IsWizard() && !IsIE11()) {
        $(document).on('s123.css.reloaded', function(event) {
            updateBarWithDefColor('modules_color_section_main', '');
            updateBarWithDefColor('modules_color_second_section_main', '');
            updateBarWithDefColor('inside_modules_color_section_box_main', '');
        });
        $(document).on('site123.colorPicker.change', function(event, id, newColor) {
            updateBarWithDefColor(id, 'input');
        });
        $(document).on('s123.boxColor.changed', function(event, moduleID) {
            var $section = $('.s123-module-percentage[data-module-id="' + moduleID + '"]');
            if ($section.length === 0)
                return;
            updateBarColorsFromClasses($section);
        });
        $(document).on('s123.sectionBackground.changed', function(event, moduleID) {
            var $section = $('.s123-module-percentage[data-module-id="' + moduleID + '"]');
            if ($section.length === 0)
                return;
            updateBarColorsFromClasses($section);
        });
        function updateBarColorsFromClasses($section) {
            $section.find('.easy-pie-chart[data-is-default-color="1"]').each(function() {
                var $chart = $(this);
                var $percentage = $chart.closest('.percentage');
                if (!$chart.data('easyPieChart')) {
                    return;
                }
                var barColor = getBarColorFromClasses($section, $percentage);
                $chart.data('easyPieChart').options['barColor'] = barColor;
                $chart.data('easyPieChart').update($chart.data('percent'));
            });
        }
        function updateBarWithDefColor(id, type) {
            if (id == 'modules_color_second_section_main') {
                $('.s123-module-percentage:not(.bg-primary).layout-1, .s123-module-percentage:not(.bg-primary).layout-3').each(function() {
                    updateBarColorsFromClasses($(this));
                });
            } else if (id == 'modules_color_section_main') {
                $('.s123-module-percentage.bg-primary.layout-1, .s123-module-percentage.bg-primary.layout-3').each(function() {
                    updateBarColorsFromClasses($(this));
                });
            } else {
                $('.inside_page .bg-primary.s123-module-percentage.layout-1, .inside_page .bg-primary.s123-module-percentage.layout-3').each(function() {
                    updateBarColorsFromClasses($(this));
                });
            }
        }
                ...
                $('#popupJobs').find('.jobsForm').each(function(index) {
                    ...
                    $form.validate({
                        ...
                        submitHandler: function(form) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: "/versions/2/include/jobsO.php",
                                ...
                                success: function(data) {
                                    ...
                                    if (dataObj.status == 'success') {
                                        ...
                                        var $sentMessage = $('<span>' + translations.error_body + '<br>' + dataObj.msg + '</span>');
                                        bootboxDialog.find('.modal-title').html(translations.error_title);
                                        bootboxDialog.find('.bootbox-body').append($sentMessage.hide());
                                        $jobsLoadingMessage.hide();
                                        $sentMessage.slideDown(200);
                                        $form.find('button:submit').prop('disabled', false);
...
function AutoSetProductsImage() {
    $(document).on("s123.page.load", function(event) {
        ...
        if ($box.length > 0) {
            if ($box.closest('.s123-module-eCommerce').length !== 0)
                return;
...
jQuery(function($) {
    GlobalLayout2ModuleInitialize();
    setTimeout(function() {
        var $sections = $('.global-layout-2');
        if ($sections.length > 0) {
            $sections.each(function(index) {
                var $s = $(this);
                var hasGlobalLayout = $s.find('.global-layout-2-container, [data-categories-filter]').length > 0;
                if (hasGlobalLayout) {
                    handleGlobalLayout2ResponsiveRedistribution($s, false);
                    var resizeTimer;
                    $(window).off('resize.globalLayout2-immediate-' + index).on('resize.globalLayout2-immediate-' + index, function() {
                        clearTimeout(resizeTimer);
                        resizeTimer = setTimeout(function() {
                            handleGlobalLayout2ResponsiveRedistribution($s, true);
                        }, 300);
                    });
                }
            });
...
function GlobalLayout2ModuleInitialize() {
    $(document).on('s123.page.ready', function(event) {
        var $sections = $('.global-layout-2');
        $sections.each(function(index) {
            var $s = $(this);
            var $categories = $s.find('.global-layout-2-category');
            var categories = new ModuleLayoutCategories({
                $items: $s.find('.global-layout-2-category'),
                $categoriesContainer: $s.find('.categories-panel'),
                $filterButton: $s.find('.items-responsive-filter'),
                $categories: $s.find('.items-categories-container li')
            });
            ...
            if (hasGlobalLayout) {
                handleGlobalLayout2ResponsiveRedistribution($s, false);
                var resizeTimer;
                $(window).off('resize.globalLayout2-' + index).on('resize.globalLayout2-' + index, function() {
                    clearTimeout(resizeTimer);
                    resizeTimer = setTimeout(function() {
                        handleGlobalLayout2ResponsiveRedistribution($s, true);
                        // Pass true for resize
                    }, 300);
...
function initializeGlobalLayout2VerticalInfiniteCarousel($section) {
    var $layoutColumns = $section.find('.global-layout-2-column');
    $layoutColumns.find('.global-layout-2-grid').css('animation-play-state', 'paused');
    if ($layoutColumns.length > 0) {
        $layoutColumns.each(function() {
            var $column = $(this);
            var $grid = $column.find('.global-layout-2-grid');
            if ($grid.length > 0 && $column.data('animate') === true) {
                var totalItems = $column.data('total-items') || 0;
                var columnNumber = $column.data('column') || 1;
                var direction = $column.data('direction') || 'up';
                var screenWidth = $(window).width();
                var currentColumnCount = screenWidth <= 767 ? 1 : (screenWidth <= 991 ? 2 : 3);
                var fastAnimationDuration = calculateGlobalLayout2ResponsiveAnimationDuration(totalItems, currentColumnCount);
                $grid.css('animation-duration', fastAnimationDuration + 's !important');
                $grid[0].style.setProperty('animation-duration', fastAnimationDuration + 's', 'important');
                var originalItems = $column.data('original-items') || 0;
                var duplicateCount = $column.data('duplicate-count') || 0;
                ...
                $column.on('touchmove', function(e) {
                ...
                $column.attr('role', 'region');
                $column.attr('aria-label', 'Scrolling content column ' + columnNumber);
                $column.on('keydown', function(e) {
                    switch (e.key) {
                    case ' ':
                    case 'Spacebar':
                        e.preventDefault();
                        if ($grid.css('animation-play-state') === 'paused') {
                            $grid.css('animation-play-state', 'running');
                        } else {
                            $grid.css('animation-play-state', 'paused');
                        }
                    ...
                    case 'ArrowDown':
                        e.preventDefault();
                        var $items = $column.find('.global-layout-2-item');
                        if ($items.length > 0) {
                            $items.first().focus();
                        }
                        break;
                    ...
                    start: function() {
                        this.checkInterval = setInterval(function() {
                            ...
                            if (performanceMonitor.frameCount % 300 === 0) {
                                ...
                                var elapsed = currentTime - performanceMonitor.startTime;
                                var fps = (performanceMonitor.frameCount / elapsed) * 1000;
                                if (fps < 30) {
                                    console.warn('Global Layout 2: Low FPS detected (' + Math.round(fps) + '), consider optimizing');
                                }
                            }
                        }, 16);
                        // ~60fps monitoring
        ...
        setTimeout(function() {
            $layoutColumns.find('.global-layout-2-grid').css('animation-play-state', 'running');
        }, 50);
        // Small delay to ensure all DOM updates are complete
    }
}
function handleGlobalLayout2ResponsiveRedistribution($section, isResize) {
    var screenWidth = $(window).width();
    var isMobile = screenWidth <= 767;
    var isTablet = screenWidth >= 768 && screenWidth <= 991;
    var isDesktop = screenWidth >= 992;
    if (!isResize && $section.data('global-layout-2-initialized')) {
        return;
    }
    var isInitialLoad = !$section.data('global-layout-2-initialized');
    var shouldRedistribute = isResize || !isDesktop;
    if (!isResize) {
        $section.data('global-layout-2-initialized', true);
    }
    if (shouldRedistribute) {
        ...
        var $categoryContainers = $section.find('[data-categories-filter]');
        $categoryContainers.each(function() {
            var $categoryContainer = $(this);
            var $columnsWrapper = $categoryContainer.find('.global-layout-2-columns-wrapper');
            if ($columnsWrapper.length === 0)
                return;
            var allItems = [];
            var $allColumns = $categoryContainer.find('.global-layout-2-column');
            $allColumns.each(function() {
                var $column = $(this);
                $column.find('.global-layout-2-item').each(function() {
                    allItems.push($(this));
                });
            });
            var uniqueItems = [];
            ...
            allItems.forEach(function($item) {
                var uniqueId = $item.data('unique-id');
                if (!uniqueId) {
                    uniqueId = $item.attr('id') || $item.find('[data-item-id]').data('item-id') || 'item-' + uniqueItems.length;
                }
                var originalId = uniqueId.toString().replace(/-copy$|-responsive-\d+$|-responsive-copy-\d+$/, '');
                if (!seenIds[originalId]) {
                    seenIds[originalId] = true;
                    var $clone = $item.clone();
                    $clone.data('unique-id', originalId);
                    uniqueItems.push({
                        id: originalId,
                        element: $clone
                    });
            ...
            redistributeGlobalLayout2Items($columnsWrapper, uniqueItems, targetColumns);
        });
        if (isInitialLoad || isResize) {
            preserveTwitterIframeStates($section);
            initializeGlobalLayout2VerticalInfiniteCarousel($section);
        }
    } else {}
}
function getGlobalLayout2Breakpoint(windowWidth) {
    if (windowWidth <= 767) {
        return 'mobile';
...
function redistributeGlobalLayout2Items($wrapper, items, columnCount) {
    var $existingColumns = $wrapper.find('.global-layout-2-column');
    $existingColumns.hide();
    var columns = [];
    ...
    if (columnCount === 1) {
        ...
        // Single column scrolls down
    } else if (columnCount === 2) {
        scrollDirections = ['up', 'down'];
        // First up, second down
    } else {
        scrollDirections = ['up', 'down', 'up'];
        // Default 3-column pattern
    }
    var hasExistingColumns = $existingColumns.length >= columnCount;
    for (var colIndex = 0; colIndex < columnCount; colIndex++) {
        var columnItems = columns[colIndex];
        if (columnItems.length === 0)
            continue;
        var scrollDirection = scrollDirections[colIndex];
        var originalItemsCount = columnItems.length;
        var setItems, duplicateCount;
        if (originalItemsCount >= 2) {
            setItems = columnItems;
            duplicateCount = 1;
            // No duplication needed
        } else {
            setItems = columnItems.concat(columnItems);
            // Duplicate to get 2 items
            duplicateCount = 2;
            // We duplicated the item once
        }
        if (setItems.length < 2) {
            setItems = setItems.concat(setItems);
            // Double the items to ensure minimum 2
        }
        var animationDuration = calculateGlobalLayout2ResponsiveAnimationDuration(setItems.length, columnCount);
        if (hasExistingColumns && colIndex < $existingColumns.length) {
            var $column = $existingColumns.eq(colIndex);
            updateGlobalLayout2Column($column, colIndex, scrollDirection, setItems, animationDuration, duplicateCount);
            $column.show();
        } else {
            var columnHtml = createGlobalLayout2ResponsiveColumnHtml(colIndex, scrollDirection, setItems, animationDuration, duplicateCount);
            $wrapper.append(columnHtml);
        }
    }
    for (var i = columnCount; i < $existingColumns.length; i++) {
        var $unusedColumn = $existingColumns.eq(i);
        $unusedColumn.hide();
    }
}
function updateGlobalLayout2Column($column, columnIndex, scrollDirection, setItems, animationDuration, duplicateCount) {
    var columnNumber = columnIndex + 1;
    var originalItemsCount = setItems.length / duplicateCount;
    var totalItems = setItems.length;
    $column.removeClass('scroll-up scroll-down static').addClass('scroll-' + scrollDirection).attr('data-column', columnNumber).attr('data-direction', scrollDirection).attr('data-animate', 'true').attr('data-animation-duration', animationDuration).attr('data-total-items', totalItems).attr('data-original-items', originalItemsCount).attr('data-duplicate-count', duplicateCount);
    var $grid = $column.find('.global-layout-2-grid');
    var $set1 = $grid.find('.item-set-1');
    var $set2 = $grid.find('.item-set-2');
    $set1.empty();
    $set2.empty();
    for (var i = 0; i < setItems.length; i++) {
        var item = setItems[i];
        var $element = item.element.clone();
        $element.attr('data-unique-id', item.id + '-responsive-' + i);
        $set1.append($element);
    }
    for (var i = 0; i < setItems.length; i++) {
        var item = setItems[i];
        var $element = item.element.clone();
        $element.attr('data-unique-id', item.id + '-responsive-copy-' + i);
        $set2.append($element);
    }
    $grid.css('animation-duration', animationDuration + 's !important');
    $grid[0].style.setProperty('animation-duration', animationDuration + 's', 'important');
}
function calculateGlobalLayout2ResponsiveAnimationDuration(totalItemsInSet, columnCount) {
    var baseSpeed = 10.08;
    // 10.08 seconds per item (20% slower than 8.4s for better readability)
    var animationDuration = (totalItemsInSet / 2) * baseSpeed;
    if (columnCount === 1) {
        animationDuration = animationDuration * 1.5;
        // 50% slower for single column (more time to read)
    } else if (columnCount === 2) {
        animationDuration = animationDuration * 1.2;
        // 20% slower for two columns
    }
    animationDuration = Math.max(20.16, animationDuration);
    // Min 20.16s (2 items * 10.08s)
    animationDuration = Math.min(animationDuration, 302.4);
    // Max 302.4s (30 items * 10.08s)
    return animationDuration;
}
function createGlobalLayout2ResponsiveColumnHtml(columnIndex, scrollDirection, setItems, animationDuration, duplicateCount) {
    var columnNumber = columnIndex + 1;
    var originalItemsCount = setItems.length / duplicateCount;
    var totalItems = setItems.length;
    var columnHtml = '<!-- Responsive Column ' + columnNumber + ': ' + originalItemsCount + ' original items × ' + duplicateCount + ' copies = ' + totalItems + ' total items, ' + animationDuration + 's duration, direction: ' + scrollDirection + ' -->' + '<div class="global-layout-2-column global-layout-2-column-' + columnNumber + ' scroll-' + scrollDirection + '" ' + 'data-column="' + columnNumber + '" ' + 'data-direction="' + scrollDirection + '" ' + 'data-animate="true" ' + 'data-animation-duration="' + animationDuration + '" ' + 'data-total-items="' + totalItems + '" ' + 'data-original-items="' + originalItemsCount + '" ' + 'data-duplicate-count="' + duplicateCount + '" ' + 'data-aos="fade-up" data-aos-delay="' + (columnIndex * 100) + '">' + '<div class="global-layout-2-grid">' + '<div class="item-set-1">';
    for (var i = 0; i < setItems.length; i++) {
        var item = setItems[i];
        var $element = item.element;
        $element.attr('data-unique-id', item.id + '-responsive-' + i);
        columnHtml += $element[0].outerHTML;
    }
    columnHtml += '</div><div class="item-set-2">';
    for (var i = 0; i < setItems.length; i++) {
        var item = setItems[i];
        var $element = item.element.clone();
        $element.attr('data-unique-id', item.id + '-responsive-copy-' + i);
...
function preserveTwitterIframeStates($section) {
    try {
        const $twitterIframes = $section.find('.s123-module-twitter-tweets iframe');
        if ($twitterIframes.length === 0) {
            return;
        }
        $twitterIframes.each(function() {
            const $iframe = $(this);
            const $tweetContainer = $iframe.closest('.twitter-tweet');
            if ($tweetContainer.length > 0) {
                $tweetContainer.attr('data-iframe-preserved', 'true');
            }
        });
    } catch (error) {}
}
window.GlobalLayout2 = {
    initialize: GlobalLayout2ModuleInitialize,
    initializeCarousel: initializeGlobalLayout2VerticalInfiniteCarousel,
    handleResponsiveRedistribution: handleGlobalLayout2ResponsiveRedistribution,
    preserveTwitterIframes: preserveTwitterIframeStates
};
var SHAPE_DIVIDERS_CONFIG = {
    '': {
        name: 'None',
        svg: '',
        mask: ''
...
var SHAPE_DIVIDERS_INDEX = {};
var SHAPE_DIVIDERS_LOADED = false;
var SHAPE_DIVIDERS_LOADING = false;
var SHAPE_DIVIDERS_CALLBACKS = [];
var SHAPE_DIVIDERS_SHAPE_LOADING = {};
var SHAPE_DIVIDERS_SHAPE_CALLBACKS = {};
var SHAPE_DIVIDERS_BASE_PATH = '/files/vendor/lw_sys/moduleStyleCustomizer/shapes';
function shapeDividers_buildGradientDefs(gradient, gradientId, forMask) {
    if (!gradient)
        return '';
    var type = gradient.type || 'linear';
    var x1 = gradient.x1 || '0%';
    var y1 = gradient.y1 || '0%';
    var x2 = gradient.x2 || '0%';
    ...
    for (var i = 0; i < stops.length; i++) {
        var stop = stops[i];
        var offset = stop.offset || '0%';
        var opacity = stop.opacity || '1';
        if (forMask) {
            defs += '<stop offset="' + offset + '" stop-color="white" stop-opacity="' + opacity + '"/>';
        } else {
            defs += '<stop offset="' + offset + '" stop-color="currentColor" stop-opacity="' + opacity + '"/>';
        }
    }
    if (type === 'linear') {
        defs += '</linearGradient>';
...
function shapeDividers_buildElement(element, forMask, gradientId) {
    ...
    case 'polygon':
        ...
        break;
    case 'circle':
        attrs = ' cx="' + (element.cx || 0) + '" cy="' + (element.cy || 0) + '" r="' + (element.r || 0) + '"';
        break;
    case 'ellipse':
        attrs = ' cx="' + (element.cx || 0) + '" cy="' + (element.cy || 0) + '" rx="' + (element.rx || 0) + '" ry="' + (element.ry || 0) + '"';
        break;
    case 'rect':
        attrs = ' x="' + (element.x || 0) + '" y="' + (element.y || 0) + '" width="' + (element.width || 0) + '" height="' + (element.height || 0) + '"';
        break;
    default:
        return '';
    }
    var fill = element.fill || null;
    if (fill === 'gradient' && gradientId) {
        attrs += ' fill="url(#' + gradientId + ')"';
    } else if (forMask) {
        attrs += ' fill="white"';
    }
    if (element.opacity !== undefined && fill !== 'gradient') {
        attrs += ' opacity="' + element.opacity + '"';
    }
    return '<' + type + attrs + '/>';
}
function shapeDividers_buildSvg(shape, shapeId) {
    var viewBox = shape.viewBox || '0 0 100 100';
    var elements = shape.elements || [];
    var gradient = shape.gradient || null;
    var gradientId = gradient ? 'shapeGrad_' + (shapeId || 'temp') : null;
    var svg = '<svg class="shapeBottom" xmlns="http://www.w3.org/2000/svg" viewBox="' + viewBox + '" preserveAspectRatio="none">';
    if (gradient) {
        svg += shapeDividers_buildGradientDefs(gradient, gradientId, false);
    }
    var elementsHtml = '';
    for (var i = 0; i < elements.length; i++) {
        elementsHtml += shapeDividers_buildElement(elements[i], false, gradientId);
    }
    svg += elementsHtml;
    svg += '</svg>';
    return svg;
}
function shapeDividers_buildMask(shape, shapeId) {
    var viewBox = shape.viewBox || '0 0 100 100';
    var elements = shape.elements || [];
    var gradient = shape.gradient || null;
    var gradientId = gradient ? 'maskGrad_' + (shapeId || 'temp') : null;
    var svg = "<svg xmlns='http://www.w3.org/2000/svg' viewBox='" + viewBox + "' preserveAspectRatio='none'>";
    if (gradient) {
        svg += shapeDividers_buildGradientDefs(gradient, gradientId, true);
    }
    var elementsHtml = '';
    for (var i = 0; i < elements.length; i++) {
        elementsHtml += shapeDividers_buildElement(elements[i], true, gradientId);
    }
    svg += elementsHtml;
    svg += '</svg>';
    var encoded = svg.replace(/%/g, '%25').replace(/</g, '%3C').replace(/>/g, '%3E').replace(/"/g, '%22').replace(/#/g, '%23').replace(/{/g, '%7B').replace(/}/g, '%7D');
    return 'url("data:image/svg+xml,' + encoded + '")';
}
function shapeDividers_loadFromJson(callback) {
    if (SHAPE_DIVIDERS_LOADED) {
        if (callback)
            callback(SHAPE_DIVIDERS_CONFIG);
        return;
    }
    if (callback) {
        SHAPE_DIVIDERS_CALLBACKS.push(callback);
    }
    if (SHAPE_DIVIDERS_LOADING) {
        return;
    }
    SHAPE_DIVIDERS_LOADING = true;
    var xhr = new XMLHttpRequest();
    xhr.open('GET', SHAPE_DIVIDERS_BASE_PATH + '/index.json', true);
        ...
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                try {
                    ...
                    SHAPE_DIVIDERS_INDEX = shapes;
                    for (var id in shapes) {
                        if (shapes.hasOwnProperty(id)) {
                            SHAPE_DIVIDERS_CONFIG[id] = {
                                name: shapes[id],
                                // Now just a string
                                svg: null,
                    ...
                    SHAPE_DIVIDERS_LOADED = true;
                } catch (e) {
                    console.warn('shapeDividersConfig: Failed to parse index JSON', e);
                }
            } else {
                console.warn('shapeDividersConfig: Failed to load index, status:', xhr.status);
            }
            SHAPE_DIVIDERS_LOADING = false;
            for (var i = 0; i < SHAPE_DIVIDERS_CALLBACKS.length; i++) {
                SHAPE_DIVIDERS_CALLBACKS[i](SHAPE_DIVIDERS_CONFIG);
            }
            SHAPE_DIVIDERS_CALLBACKS = [];
        }
    }
    ;
    xhr.send();
}
function shapeDividers_loadShape(shapeId, callback) {
    if (shapeId === '' || shapeId === null || shapeId === undefined) {
        if (callback)
            callback(SHAPE_DIVIDERS_CONFIG['']);
        return;
    }
    shapeId = shapeId.toString();
    if (SHAPE_DIVIDERS_CONFIG[shapeId] && SHAPE_DIVIDERS_CONFIG[shapeId].mask !== null) {
        if (callback)
            callback(SHAPE_DIVIDERS_CONFIG[shapeId]);
        return;
    }
    if (callback) {
        if (!SHAPE_DIVIDERS_SHAPE_CALLBACKS[shapeId]) {
            SHAPE_DIVIDERS_SHAPE_CALLBACKS[shapeId] = [];
        }
        SHAPE_DIVIDERS_SHAPE_CALLBACKS[shapeId].push(callback);
    }
    if (SHAPE_DIVIDERS_SHAPE_LOADING[shapeId]) {
        return;
    }
    SHAPE_DIVIDERS_SHAPE_LOADING[shapeId] = true;
    var xhr = new XMLHttpRequest();
    xhr.open('GET', SHAPE_DIVIDERS_BASE_PATH + '/data/' + shapeId + '.json', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState === 4) {
            var shapeConfig = null;
            if (xhr.status === 200) {
                try {
                    var shapeData = JSON.parse(xhr.responseText);
                    shapeConfig = {
                        name: shapeData.name || 'Shape ' + shapeId,
                        viewBox: shapeData.viewBox || '0 0 100 100',
                        category: shapeData.category || 'uncategorized',
                        svg: shapeDividers_buildSvg(shapeData, shapeId),
                        mask: shapeDividers_buildMask(shapeData, shapeId)
                    };
                    SHAPE_DIVIDERS_CONFIG[shapeId] = shapeConfig;
                } catch (e) {
                    console.warn('shapeDividersConfig: Failed to parse shape JSON for ID ' + shapeId, e);
                }
            } else {
                console.warn('shapeDividersConfig: Failed to load shape ' + shapeId + ', status:', xhr.status);
            }
            SHAPE_DIVIDERS_SHAPE_LOADING[shapeId] = false;
            var callbacks = SHAPE_DIVIDERS_SHAPE_CALLBACKS[shapeId] || [];
            SHAPE_DIVIDERS_SHAPE_CALLBACKS[shapeId] = [];
            for (var i = 0; i < callbacks.length; i++) {
                callbacks[i](shapeConfig);
            }
        }
    }
    ;
...
function shapeDividers_getShape(shapeId, callback) {
    shapeDividers_loadShape(shapeId, callback);
}
function shapeDividers_isShapeLoaded(shapeId) {
    if (shapeId === '' || shapeId === null || shapeId === undefined) {
        return true;
    }
    return SHAPE_DIVIDERS_CONFIG[shapeId] && SHAPE_DIVIDERS_CONFIG[shapeId].mask !== null;
}
function shapeDividers_getAllShapeIds() {
    var ids = [];
    for (var id in SHAPE_DIVIDERS_INDEX) {
        if (SHAPE_DIVIDERS_INDEX.hasOwnProperty(id)) {
            ids.push(id);
        }
    }
...
function shapeDividers_getShapesForPage(pageIndex, perPage) {
    perPage = perPage || 15;
    var allIds = shapeDividers_getAllShapeIds();
    var start;
    var count;
    if (pageIndex === 0) {
        start = 0;
        ...
        start = (perPage - 1) + (pageIndex - 1) * perPage;
        count = perPage;
    }
...
function shapeDividers_getTotalPages(perPage) {
    perPage = perPage || 15;
    var allIds = shapeDividers_getAllShapeIds();
    var totalShapes = allIds.length;
    if (totalShapes === 0)
        return 1;
    var firstPageShapes = perPage - 1;
    var remainingShapes = totalShapes - firstPageShapes;
    if (remainingShapes <= 0)
        return 1;
    return 1 + Math.ceil(remainingShapes / perPage);
}
function shapeDividers_getPageForShape(shapeId, perPage) {
    perPage = perPage || 15;
    var allIds = shapeDividers_getAllShapeIds();
    var index = allIds.indexOf(shapeId.toString());
    if (index === -1)
        return 0;
    var firstPageShapes = perPage - 1;
    if (index < firstPageShapes) {
        return 0;
        // Shape is on first page
    }
    var remainingIndex = index - firstPageShapes;
    return 1 + Math.floor(remainingIndex / perPage);
}
function shapeDividers_preloadPage(pageIndex, perPage, callback) {
    var shapeIds = shapeDividers_getShapesForPage(pageIndex, perPage);
    var loaded = 0;
    var total = shapeIds.length;
    if (total === 0) {
        if (callback)
            callback();
        return;
    ...
    for (var i = 0; i < shapeIds.length; i++) {
        shapeDividers_loadShape(shapeIds[i], function() {
            loaded++;
...
function shapeDivider_isRTLMode() {
    var html_dir;
    if (window.topWindow && window.topWindow.Wizard && window.topWindow.Wizard.Preview && window.topWindow.Wizard.Preview.ready) {
        html_dir = window.topWindow.Wizard.Preview.$('html').attr('dir');
    } else {
        html_dir = $('html').attr('dir');
    }
    return html_dir === 'rtl';
}
function shapeDivider_flipMaskForRTL(maskUrl) {
    if (!maskUrl || maskUrl === '')
        return maskUrl;
    var svgMatch = maskUrl.match(/url\("data:image\/svg\+xml,([^"]+)"\)/);
    if (!svgMatch)
        return maskUrl;
    var encodedSVG = svgMatch[1];
    var decodedSVG = decodeURIComponent(encodedSVG);
    var viewBoxMatch = decodedSVG.match(/viewBox=['"]([^'"]+)['"]/);
    var translateX = -100;
    // Default
    if (viewBoxMatch) {
        var viewBoxParts = viewBoxMatch[1].split(/\s+/);
        if (viewBoxParts.length >= 3) {
            translateX = -parseFloat(viewBoxParts[2]);
            // Use viewBox width
        }
    }
    var flippedSVG = decodedSVG.replace(/<svg([^>]*)>/, '<svg$1><g transform="scale(-1, 1) translate(' + translateX + ', 0)">').replace(/<\/svg>/, '</g></svg>');
    var encodedFlippedSVG = encodeURIComponent(flippedSVG);
    return 'url("data:image/svg+xml,' + encodedFlippedSVG + '")';
}
function shapeDivider_getRTLAdjustedMask(shapeId, maskUrl) {
    var isRTL = shapeDivider_isRTLMode();
    if (!isRTL) {
        return maskUrl;
        // LTR - no adjustment needed
    }
    var rtlShapeMap = {
        '3': '5',
    ...
    if (rtlShapeMap[shapeId]) {
        var rtlShapeId = rtlShapeMap[shapeId];
        var rtlShapeConfig = SHAPE_DIVIDERS_CONFIG[rtlShapeId];
        if (rtlShapeConfig && rtlShapeConfig.mask) {
            return rtlShapeConfig.mask;
        }
    }
    return shapeDivider_flipMaskForRTL(maskUrl);
}
function shapeDivider_getRTLAdjustedMaskAsync(shapeId, maskUrl, callback) {
    var isRTL = shapeDivider_isRTLMode();
    if (!isRTL) {
        callback(maskUrl);
        // LTR - no adjustment needed
    ...
    if (rtlShapeMap[shapeId]) {
        var rtlShapeId = rtlShapeMap[shapeId];
        shapeDividers_loadShape(rtlShapeId, function(rtlConfig) {
            if (rtlConfig && rtlConfig.mask) {
                callback(rtlConfig.mask);
            } else {
...
jQuery(function($) {
    $(document).on('s123.page.ready', function(event) {
        shapeDivider_initAll();
    });
});
function shapeDivider_getSectionsInVisualOrder() {
    var $sections = $('section.s123-module');
    var sections_arr = $sections.toArray().sort(function(a, b) {
        var order_a_num = parseInt($(a).css('order')) || 0;
        var order_b_num = parseInt($(b).css('order')) || 0;
        return order_a_num - order_b_num;
    });
    return $(sections_arr);
}
function shapeDivider_getVisualNextSection($section) {
    var $sections_ordered = shapeDivider_getSectionsInVisualOrder();
    var current_index_num = -1;
    $sections_ordered.each(function(index) {
        if ($(this).data('module-id') === $section.data('module-id')) {
            current_index_num = index;
            return false;
            // break
        }
    });
    if (current_index_num >= 0 && current_index_num < $sections_ordered.length - 1) {
        return $sections_ordered.eq(current_index_num + 1);
    }
    return $();
}
function shapeDivider_initAll() {
    if (typeof IsInsidePage === 'function' && typeof IsMultiPagesV2 === 'function' && typeof IsDataPage === 'function') {
        var isInsidePage = IsInsidePage();
        var isMultiPagesV2 = IsMultiPagesV2();
        var isDataPage = IsDataPage();
        var shouldDisableShapes = isDataPage || (isInsidePage && !isMultiPagesV2);
        if (shouldDisableShapes) {
            $('section[data-shape-divider]').each(function() {
                var $section = $(this);
                $section.removeAttr('data-shape-divider');
                $section.removeAttr('data-shape-height');
                $section[0].style.removeProperty('--shape-mask');
                $section[0].style.removeProperty('--shape-height');
                $section[0].style.removeProperty('--shape-height-mobile');
                $section.css('z-index', '');
    ...
    $('section.after-shape-divider').each(function() {
        var $section = $(this);
        $section.removeClass('after-shape-divider');
        $section.css('z-index', '');
        $section.css('position', '');
        $section[0].style.removeProperty('--prev-shape-height');
        $section[0].style.removeProperty('--prev-shape-height-tablet');
        $section[0].style.removeProperty('--prev-shape-height-mobile');
    });
    $('section[data-shape-divider]').each(function() {
        var $section = $(this);
        $section.css('z-index', '');
        $section.css('position', '');
        $section[0].style.removeProperty('--shape-height-tablet');
        $section[0].style.removeProperty('--shape-height-mobile');
    });
    var $footer = $('footer.global_footer');
    if ($footer.length > 0) {
        $footer.removeClass('after-shape-divider');
        $footer[0].style.removeProperty('--prev-shape-height');
        $footer[0].style.removeProperty('--prev-shape-height-tablet');
        $footer[0].style.removeProperty('--prev-shape-height-mobile');
    }
    var base_zindex_num = 100;
    var current_zindex_num = base_zindex_num;
    var $sections_ordered = shapeDivider_getSectionsInVisualOrder();
    $sections_ordered.each(function() {
        var $section = $(this);
        if ($section.attr('data-shape-divider')) {
            current_zindex_num = shapeDivider_initSection($section, current_zindex_num);
            current_zindex_num--;
        }
    });
    if ($sections_ordered.length > 0) {
        var $last_section = $sections_ordered.last();
        if ($last_section.attr('data-shape-divider')) {
            var shape_height_num = parseInt($last_section.attr('data-shape-height')) || 100;
            var shape_height_tablet_num = Math.round(shape_height_num / 2);
            var shape_height_mobile_num = Math.round(shape_height_num / 4);
            if ($footer.length > 0) {
                $footer[0].style.setProperty('--prev-shape-height', shape_height_num + 'px');
                $footer[0].style.setProperty('--prev-shape-height-tablet', shape_height_tablet_num + 'px');
                $footer[0].style.setProperty('--prev-shape-height-mobile', shape_height_mobile_num + 'px');
                $footer.addClass('after-shape-divider');
            }
        }
    }
}
function shapeDivider_initSection($section, zindex_num) {
    var shape_id_txt = $section.attr('data-shape-divider');
    var shape_height_num = parseInt($section.attr('data-shape-height')) || 100;
    var shape_height_tablet_num = Math.round(shape_height_num / 2);
    var shape_height_mobile_num = Math.round(shape_height_num / 4);
    if (typeof zindex_num !== 'undefined') {
        $section.css('z-index', zindex_num);
        $section.css('position', 'relative');
    }
    var $next_section = shapeDivider_getVisualNextSection($section);
    if ($next_section.length > 0) {
        $next_section[0].style.setProperty('--prev-shape-height', shape_height_num + 'px');
        $next_section[0].style.setProperty('--prev-shape-height-tablet', shape_height_tablet_num + 'px');
        $next_section[0].style.setProperty('--prev-shape-height-mobile', shape_height_mobile_num + 'px');
        $next_section.addClass('after-shape-divider');
        if (typeof zindex_num !== 'undefined') {
            zindex_num--;
            $next_section.css('z-index', zindex_num);
            $next_section.css('position', 'relative');
        }
    }
    shapeDividers_loadShape(shape_id_txt, function(shape_config) {
        if (!shape_config || !shape_config.mask)
            return;
        shapeDivider_getRTLAdjustedMaskAsync(shape_id_txt, shape_config.mask, function(mask_url_txt) {
            $section[0].style.setProperty('--shape-mask', mask_url_txt);
            $section[0].style.setProperty('--shape-height', shape_height_num + 'px');
            $section[0].style.setProperty('--shape-height-tablet', shape_height_tablet_num + 'px');
            $section[0].style.setProperty('--shape-height-mobile', shape_height_mobile_num + 'px');
        });
    });
    return zindex_num;
}
function shapeDivider_updateSection($section, options) {
    ...
    options = options || {};
    var shapeId = options.shapeId !== undefined ? options.shapeId : $section.attr('data-shape-divider');
    var height = parseInt(options.height) || parseInt($section.attr('data-shape-height')) || 100;
    var skipZIndex = options.skipZIndex || false;
    var affectedElements = {
        section: $section,
        previousNextSection: null,
        newNextSection: null
    };
    var $previousNextSection = $section.hasClass('after-shape-divider') ? null : shapeDivider_getVisualNextSection($section);
    var currentHeight = $section[0].style.getPropertyValue('--shape-height');
    if (currentHeight) {
        var $potentialPrevNext = shapeDivider_getVisualNextSection($section);
        if ($potentialPrevNext.length > 0 && $potentialPrevNext.hasClass('after-shape-divider')) {
            affectedElements.previousNextSection = $potentialPrevNext;
        }
    }
    if (!shapeId || shapeId === '') {
        $section.removeAttr('data-shape-divider');
        ...
        if (affectedElements.previousNextSection && affectedElements.previousNextSection.length > 0) {
            affectedElements.previousNextSection.removeClass('after-shape-divider');
            affectedElements.previousNextSection[0].style.removeProperty('--prev-shape-height');
            affectedElements.previousNextSection[0].style.removeProperty('--prev-shape-height-mobile');
            if (!skipZIndex) {
                affectedElements.previousNextSection.css('z-index', '');
                affectedElements.previousNextSection.css('position', '');
            }
        }
        return {
            success: true,
    ...
    var mobileHeight = Math.round(height / 4);
    $section.attr('data-shape-divider', shapeId);
    $section.attr('data-shape-height', height);
    $section[0].style.setProperty('--shape-height', height + 'px');
    $section[0].style.setProperty('--shape-height-mobile', mobileHeight + 'px');
    var maskUrl = options.mask;
    if (maskUrl) {
        $section[0].style.setProperty('--shape-mask', maskUrl);
    } else {
        shapeDividers_loadShape(shapeId, function(shapeConfig) {
            if (!shapeConfig || !shapeConfig.mask) {
                return;
            }
            shapeDivider_getRTLAdjustedMaskAsync(shapeId, shapeConfig.mask, function(adjustedMaskUrl) {
                $section[0].style.setProperty('--shape-mask', adjustedMaskUrl);
            });
        });
    }
    if (!skipZIndex) {
        var zIndex = shapeDivider_calculateZIndex($section);
        $section.css('z-index', zIndex);
        $section.css('position', 'relative');
    }
    if (affectedElements.previousNextSection && affectedElements.previousNextSection.length > 0) {
        var $newNextSection = shapeDivider_getVisualNextSection($section);
        if (!$newNextSection.is(affectedElements.previousNextSection)) {
            affectedElements.previousNextSection.removeClass('after-shape-divider');
            affectedElements.previousNextSection[0].style.removeProperty('--prev-shape-height');
            affectedElements.previousNextSection[0].style.removeProperty('--prev-shape-height-tablet');
            affectedElements.previousNextSection[0].style.removeProperty('--prev-shape-height-mobile');
            if (!skipZIndex) {
                affectedElements.previousNextSection.css('z-index', '');
                affectedElements.previousNextSection.css('position', '');
    ...
    var $nextSection = shapeDivider_getVisualNextSection($section);
    affectedElements.newNextSection = $nextSection;
    var tabletHeight = Math.round(height / 2);
    if ($nextSection.length > 0) {
        $nextSection[0].style.setProperty('--prev-shape-height', height + 'px');
        $nextSection[0].style.setProperty('--prev-shape-height-tablet', tabletHeight + 'px');
        $nextSection[0].style.setProperty('--prev-shape-height-mobile', mobileHeight + 'px');
        $nextSection.addClass('after-shape-divider');
        if (!skipZIndex) {
            var nextZIndex = shapeDivider_calculateZIndex($section) - 1;
            $nextSection.css('z-index', nextZIndex);
    ...
    if ($footer.length > 0) {
        var $sections_ordered = shapeDivider_getSectionsInVisualOrder();
        var $lastSection = $sections_ordered.last();
        if ($lastSection.is($section)) {
            $footer[0].style.setProperty('--prev-shape-height', height + 'px');
            $footer[0].style.setProperty('--prev-shape-height-tablet', tabletHeight + 'px');
            $footer[0].style.setProperty('--prev-shape-height-mobile', mobileHeight + 'px');
            $footer.addClass('after-shape-divider');
        }
    }
    return {
...
function shapeDivider_calculateZIndex($section) {
    var baseZIndex = 100;
    var $allShapeDividerSections = $('section[data-shape-divider]');
    var $sectionsOrdered = shapeDivider_getSectionsInVisualOrder();
    var shapeDividerOrder = [];
    $sectionsOrdered.each(function() {
        if ($(this).attr('data-shape-divider')) {
            shapeDividerOrder.push($(this).attr('id') || $(this).attr('data-module-id'));
        }
    });
    var sectionId = $section.attr('id') || $section.attr('data-module-id');
    var position = shapeDividerOrder.indexOf(sectionId);
    if (position === -1) {
        return baseZIndex;
    }
...
(function() {
    function initShapeDividerRTLUpdates() {
        if (window.$) {
        ...
        if (window.topWindow && window.topWindow.Wizard && window.topWindow.Wizard.Preview) {
            var lastPreviewHtmlDir = null;
            var Preview = window.topWindow.Wizard.Preview;
            var checkAndUpdatePreviewShapeDividers = function() {
                if (Preview.ready && Preview.$) {
                    var previewHtml = Preview.$('html')[0];
                    if (previewHtml) {
                        var currentDir = previewHtml.getAttribute('dir');
                        if (currentDir !== lastPreviewHtmlDir) {
                            lastPreviewHtmlDir = currentDir;
                            shapeDivider_initAll();
                        }
                    }
                }
            };
            var $iframe = window.topWindow.$('#websitePreviewIframe');
            if ($iframe && $iframe.length) {
                $iframe.on('load.shapeDividerRTL', function() {
                    setTimeout(checkAndUpdatePreviewShapeDividers, 50);
                });
            }
            setInterval(checkAndUpdatePreviewShapeDividers, 100);
            setTimeout(checkAndUpdatePreviewShapeDividers, 100);
        }
    }
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initShapeDividerRTLUpdates);
    } else {
        initShapeDividerRTLUpdates();
    }
}
)();
